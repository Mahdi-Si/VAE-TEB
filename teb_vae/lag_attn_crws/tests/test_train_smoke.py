...
**What this file does not assert is the diagnostic page's contents.** The shipped forecast rows walk
a dense $(T_{\mathrm{valid}}, H, R)$ block while this model emits $(A_{\max}, H, R)$ indexed by
position in the decoded set, and the shipped input rows and run-level budget figure are built from
the production two-sided filter bank, which does not describe these channels. All three failures are
swallowed by design -- a figure is never worth failing a multi-day fit for -- so what is asserted
here is what must hold whatever the page did: the callback is attached, the fit finished, and no
exception escaped the validation epoch it draws on.
"""
...
#: cancellation rather than magnitude -- ``pred_gap`` is the difference of two $243$-nat block
#: scores and moves by $6 \cdot 10^{-4}$ on a magnitude below $1$ -- and the relative part covers
#: ``grad_norm``, which moves by $0.13$ on $441$. See the test that reads them.
_DRIFT_ATOL = 5.0e-3
_DRIFT_RTOL = 2.0e-3
...
def test_the_spike_breaker_never_latched(fit):
    """The inherited ``additive_margin`` is stated in nats of a block twice this one's, so the live
    risk while it stands is a margin that never fires -- but a margin that skipped ordinary batches
    would read in a log exactly like a model that keeps blowing up, and only this column separates
    them."""
    driver, _ = fit
...
def test_the_run_trains_at_the_budgets_width_and_the_configs_tiling(fit):
    ...
    the budget resolves to, which is the one thing that separates this cell from the causal-feature
...
def test_the_five_target_channel_columns_are_absent_rather_than_vacuous(fit):
    """The five readouts the causal-feature cell reports and this one drops. Every one partitions
    kept **target** channels, and this target has none -- its last axis counts raw samples, which
    have no warm-up, no filter and no order to rank by -- so a re-pointed column would be a
    measurement of nothing that reads exactly like a measurement."""
    driver, _ = fit
    frame = _metrics(driver)
...
def test_the_headline_gap_is_recorded_on_both_stages_and_is_finite(fit):
    """The readout this cell exists to produce: the same quantity the comparison model reports, over
    the same raw target, computed from inputs that do not contain the answer.
...
def test_the_two_source_warmth_columns_are_fractions_and_move_together(fit):
...
def test_the_two_readouts_the_beta_pair_is_answered_from_are_present_and_finite(fit):
    r"""$\beta = 1.0$ and $\beta_p = 0.1$ are inherited from a model whose block was twice this one's
    and whose anchor count was $24\times$ larger, so whether they transfer is an open question rather
    than a settled one -- and these are the two columns it is answered from. Asserted present and
    finite rather than inside a band: a band would be this file inventing the answer."""
    driver, _ = fit
    frame = _metrics(driver)
...
def test_the_checkpoint_is_written_under_this_models_stem(fit):
    """Four models writing ``lag-attn-rws-epoch=00.ckpt`` into a shared directory would be
    ...
    assert all(path.name.startswith("lag-attn-crws-epoch=") for path in checkpoints), [
...
def test_the_plotting_callback_is_attached_and_the_fit_survives_whatever_it_drew(fit):
    ...
    this file's claim: the shipped forecast rows index an anchor into a dense block's first axis and
    this model's forecast is indexed by position in the decoded set, so the rows this cell needs are
    its own and are asserted where they are written."""
    driver, trainer = fit
...
def test_under_the_shipped_settings_the_geometry_columns_are_still_exact(tmp_path_factory, fit):
    ...
    describe both. Measured across two fits of this config:

    * ``train/grad_norm`` moves by $0.13$ on a magnitude of $441$ -- $3$ parts in $10^{4}$, the
      largest *relative* movement, and simply the largest number the run reports;
    * ``train/pred_gap`` moves by $6 \cdot 10^{-4}$ on a magnitude below $1$ -- the largest
      *absolute* movement of any small column, and cancellation rather than noise: it is the
      difference of two $\approx 243$-nat block scores that each moved by $2 \cdot 10^{-3}$.

    So a single absolute tenth of a nat -- what this bound was before it was measured against a run
    that draws its diagnostic page -- is vacuous for the first and unmeetable for the second, and
    :data:`_DRIFT_ATOL` and :data:`_DRIFT_RTOL` carry roughly an order of magnitude over each.
    """

...
.. code-block:: bash
    ...
    python -m teb_vae.lag_attn_cfs.eval.run --only cross_subgroup --output-dir <a finished run>
...
from teb_vae.lag_attn_cfs.eval import cohort, figures_seam as figures
...
METRIC_SOURCES: Tuple[MetricSource, ...] = (
    ...
    # --- This cell's own, and the reason each is worth a place in the family ------------------
    #
    # The three warm-up tertiles rather than one summary of them. They already recompose to
    # ``pred_gap``, which is registered above, so a cohort that differs in the *total* is visible
    # there; what only these three can show is a cohort that differs in **which channels** the gap
    # came from -- the slow ones the causal front end warms last, or the fast ones it warms first.
    # That is a statement about where a cohort's forecastability lives, and collapsing them would
    # remove exactly it.
    MetricSource(
        "warmup", "warmup_per_recording.csv", "pred_gap_warm_lo", higher_is_better=True
    ),
    MetricSource(
        "warmup", "warmup_per_recording.csv", "pred_gap_warm_mid", higher_is_better=True
    ),
    MetricSource(
        "warmup", "warmup_per_recording.csv", "pred_gap_warm_hi", higher_is_better=True
    ),
    # The availability-clock difference, and the one metric here no other cell in the grid has.
    # ``coupling_minus_clock`` rather than ``source_conditioned_kl_raw`` alone: the source
    # availability pattern is a deterministic function of $t$ and identical in every row, so a
    # cohort difference in the raw coupling can be a difference in how much *record* each cohort
    # has rather than in how much source information the model found. The difference is the part
    # attributable to source variation, and it is the one that answers the clinical question.
    MetricSource(
        "source_null", "source_null_per_recording.csv", "coupling_minus_clock",
        higher_is_better=True,
    ),
    # The band-resolved readout that replaces the sibling's frequency-domain pair. One band rather
    # than five, for the reason the list is short at all: every band added widens the Holm family
    # that every other metric here is corrected against. ``variability`` is the band chosen --
    # the largest by channel count and the one a fetal-monitoring reader asks about first -- and
    # ``test_the_band_named_here_is_a_real_clinical_band`` keeps the string honest against
    # :data:`~teb_vae.lag_attn.eval.band_partition.CLINICAL_BANDS`.
    #
    # A *gap* rather than a skill ratio, deliberately, and for the same reason the sibling
    # registered a residual rather than a spectral gain: this column has a direction a table can
    # publish, and the per-band skill ratio's own denominator moves with the cohort.
    MetricSource(
...
#: constant is checkable against the shared band table rather than being a string only this
#: module and the analysis that writes the column agree on.
SPECTRAL_SKILL_BAND = "variability"
...
def usable_groups(
    ...
    Returns:
        ...
        worst-first rather than alphabetically. That order is load-bearing rather than
        presentational: ``stats.pairwise_comparisons`` names each pair in the order it receives
        them, so handing it severity-descending cohorts is what makes every comparison read
        *more severe against less severe*. The exclusion is returned rather than discarded
...
def _draw_effect_heatmap(figure: Any, ax: Any, record: Dict[str, Any]) -> None:
    ...
    cohort sit together and the HIE pairs sit left. The orientation *inside* a column is that
    same order seen from the other side: ``usable_groups`` hands the cohorts to
    ``pairwise_comparisons`` severity-descending and that sweep names each pair in the order it
    receives, so a column reads more severe against less severe and its delta carries the sign
    convention every other column carries -- positive means the more severe cohort runs higher.

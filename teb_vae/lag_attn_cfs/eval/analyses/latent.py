...
DIAGNOSTIC_COLUMNS: Tuple[Tuple[str, str], ...] = (
    ...
    # "coefficients" rather than the sibling's "raw samples", and the *only* wording this module
    # changes. It is not cosmetic: the sentence is written into ``latent_diagnostics.csv``, there
    # is no raw sample anywhere in this pipeline for a log-variance to be per, and the two
    # denominators differ -- so a reader comparing this row against the sibling's would be
    # comparing per-sample and per-coefficient figures under one description.
    ("mean_logvar_full", "mean decoder log-variance over the scored target coefficients"),
    (

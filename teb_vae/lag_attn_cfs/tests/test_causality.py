...
The dataset's causality is a property of its filters and is asserted where the shards are built. What
is asserted here is the *network's*: that anchor $t$'s outputs are a function of steps up to $t$ and
of nothing after. Both halves are needed, and neither implies the other -- a transform that only
...
neither reads the data nor depends on where the sequence ends. Asserted rather than argued: the
tests below run on the *guarded* model, which is the configuration in which a length-dependent mask
would show.

``causal_norm: true`` is the qualifier, exactly as on the conv-LSTM feature sibling: these encoders
carry a time-pooling normaliser whose statistics would otherwise run over the whole sequence, and the
flag causalises them. The paired negative test records that the claim is false without it, so the
...
def _history_states(model, streams, length: int):
    ...
    at the decode -- which is the right behaviour and not the property under test. What prefix
    equivalence is about is the *history*, and the warm-up mask sits inside it.
...
def test_a_shorter_sequence_is_refused_rather_than_decoded_at_a_shifted_anchor_set(
    ...
    Stated as a test because the base's dense slice would simply return fewer anchors on a short
    tensor, silently; a gather at construction-time indices raises instead.
    """
    streams = make_streams(kwargs)

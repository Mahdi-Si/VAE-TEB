...
from teb_vae.lag_attn.nets.controls import make_derangement, resolve_perm_index

__all__ = ["make_derangement", "resolve_perm_index", "perm_forward_outputs"]
...
def perm_forward_outputs(
    ...
    *,
    ...
    Returns:
        A shallow copy of ``forward_outputs`` with ``mu_post``, ``logvar_post``, ``z_post``,
        ``attn_weights``, ``mu_full`` and ``logvar_full`` recomputed under $\pi(U)$, plus the
        ``perm_index`` used. The base branch (``mu_base``, ``logvar_base``) and the prior are the

...
with being correct. That vacuity is itself a passing test below, rather than a comment.
"""
...
#: scoring the shuffled forecast against a different anchor set than the matched one. Under the
#: anchored raw gather that is sharper than it is on the feature cells: a rebuilt anchor set would
#: make the two branches score two different raw windows, with every shape correct.
_UNTOUCHED_KEYS = (
    "mu_prior",
    "logvar_prior",
    "raw_logvar_prior",
...
def _model(perturb_posterior=None, **overrides) -> SeqVaeLagAttnTrfCrws:
    torch.manual_seed(0)
...
# =================================================================================================
# The whole file is vacuous at initialisation, and that is a passing test
# =================================================================================================
def test_at_initialisation_the_deranged_source_produces_the_matched_posterior() -> None:
    """Why every assertion below perturbs first, stated as a measurement rather than as a comment.

    The posterior delta heads are zero-initialised, so a *stranger's* source produces bitwise the
    same posterior as the sample's own -- and therefore the same KL, the same shuffled KL and a
    ``shuffle_penalty`` that is a difference of two scores of one distribution. Every test below
    would pass on a model that ignores its source entirely.

    The claim is about the two distribution parameters and not about ``mu_full``, which is where the
    vacuity is easy to state wrongly: the control decodes a fresh $\\epsilon$, so the shuffled
    forecast moves at initialisation for a reason that has nothing to do with the source.
    """
    model = _model()
    ...
    assert torch.equal(permuted["mu_post"], out["mu_post"])
    assert torch.equal(permuted["logvar_post"], out["logvar_post"])
    assert torch.equal(permuted["mu_post"], out["mu_prior"]), "the posterior is not yet the prior"


# =================================================================================================
# The shape, explicitly
# =================================================================================================
def test_the_permuted_decode_has_the_same_shape_as_the_matched_one(perturb_posterior) -> None:
...
def test_a_control_given_no_anchors_would_decode_a_different_shape(perturb_posterior) -> None:
...
# =================================================================================================
def test_the_source_free_tensors_are_the_same_objects(perturb_posterior) -> None:
...
def test_the_source_driven_tensors_are_genuinely_rebuilt(perturb_posterior) -> None:
...
def test_the_attention_is_rebuilt_under_this_models_own_lag_mask(perturb_posterior) -> None:
    """The control poses the query and attends again, and it must do so under the model's own mask
    -- reached through ``build_lag_mask``, which this input domain overrides for the lag floor. A
...
# =================================================================================================
def test_the_three_readouts_appear_on_validation_only(task, perturb_posterior) -> None:
...
def test_the_task_hands_the_control_the_matched_forwards_anchors(task, perturb_posterior) -> None:
...
def test_a_degenerate_batch_cannot_be_deranged(perturb_posterior) -> None:
...
def test_the_control_is_skipped_on_a_degenerate_validation_batch(task, perturb_posterior) -> None:
...
def test_the_control_uses_a_real_derangement(perturb_posterior) -> None:

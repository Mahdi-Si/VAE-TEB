...
# The rank statistics live one layer down, in ``eval/stats.py``, because the KLD-to-delivery
# analysis needs the same Holm / Kruskal-Wallis / pairwise machinery and an analysis may never
# import from a sibling. Re-imported here (rather than referenced through ``stats.``) so the names
# stay module attributes -- ``cross_subgroup.holm_adjust`` and the rest remain the tested surface.
from teb_vae.lag_attn.eval.stats import (  # noqa: F401 - re-exported for the module's public API
    DELTA_THRESHOLDS,
    MIN_GROUP_SIZE,
    cliffs_delta,
    delta_magnitude,
...
# Statistics
# ---------------------------------------------------------------------------
# ``holm_adjust``, ``cliffs_delta``, ``delta_magnitude``, ``kruskal_across_groups`` and
# ``pairwise_comparisons`` are imported from ``eval/stats.py`` at the top of this module -- they
# are shared verbatim with the KLD-to-delivery analysis. Only the frame-to-groups helper below is
# specific to this analysis, so it stays here.
def _usable_groups(

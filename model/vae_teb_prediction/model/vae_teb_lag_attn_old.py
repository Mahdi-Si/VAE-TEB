"""Lag-Attentive Residual VAE-TEB (v1) — FROZEN LEGACY COPY (pre-refactor).

.. warning::
   This is a verbatim snapshot of ``vae_teb_lag_attn_v1.py`` at commit
   ``375b50d^`` (the state immediately *before* "Improve core vae-teb lag
   attn model architecture."). It exists solely so that checkpoints trained
   before that refactor (e.g. the ``2026-05-01`` ``lag_attn_v1`` runs) can
   still be loaded for testing. Its decoders own their horizon refinement
   directly (``horizon_embedding`` / ``refine`` (``conv_1/norm_1/conv_2/norm_2``)
   / ``out_norm``) and the encoders use ``conv_1/conv_2/conv_3`` rather than the
   shared ``HorizonDecoderCore`` + ``ModuleList`` layout of the current model.
   Do not develop against this file; use ``vae_teb_lag_attn_v1.py`` for new work.

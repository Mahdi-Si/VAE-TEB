...
The claim is inherited, and this cell changes more than the two-sided one did, so the question of
whether it survives is a live one on four counts:
...
* the decoder log-variance head widened to $C_{\mathrm{keep}} = 98$ and is consumed only under
  ``gaussian_nll``, while ``tiny.yaml`` ships ``mse``;
...
def trainer(tmp_path):
    ...
    """
    driver = LagAttnCfsTrainer(config_file_path=str(_CONFIG))
...
def test_the_hook_is_the_un_prefixed_name_the_framework_looks_up():
    ...
    run."""
    assert "_select_ddp_strategy" not in vars(LagAttnCfsTrainer)
    assert LagAttnCfsTrainer.select_ddp_strategy is LagAttnRwsTrainer.select_ddp_strategy
...
def test_the_unaligned_arm_builds_none_and_dropping_the_first_source_block_would(tmp_path):
    ...
    torch.manual_seed(0)
    unaligned = SeqVaeLagAttnCfs(**shipped_warmup_kwargs(align=False))
...
def test_the_phase_derivation_runs_no_collective():
    ...
    source = inspect.getsource(SeqVaeLagAttnCfsTask.anchor_phase)
    source += inspect.getsource(SeqVaeLagAttnCfsTask.resolve_anchor_geometry)
    source += inspect.getsource(SeqVaeLagAttnCfsTask._build_forward_inputs)
...
def test_the_only_collective_in_the_step_is_the_inherited_permutation_decision():
    ...
    assert "_sync_perm_decision" not in vars(SeqVaeLagAttnCfsTask)
...
def test_the_anchor_tensor_shape_is_a_geometry_constant_at_every_phase(phase):
    ...
    torch.manual_seed(0)
    model = SeqVaeLagAttnCfs(**tiny_warmup_kwargs(anchor_stride=TINY_STRIDE)).eval()

...
.. code-block:: bash
    ...
    # Dev-box validation run over the committed HIE shard
    python -m teb_vae.lag_attn_transformer_fs.trainer \
...
against.** The config-to-constructor sweep, the causal-reach-budget resolution, the four pre-flight
guards, the callback assembly, the resolved-config persistence, the DDP strategy selection, the
step-granular learning-rate monitor and the diagnostic-plot wiring all reach this class through
:class:`~teb_vae.lag_attn_fs.trainer.LagAttnFsTrainer` and
...
Two consequences of the resolution order are worth reading twice, because neither is written
anywhere else:

* ``TARGET_FIELDS`` and ``TRACKED_METRICS`` come from the **feature** parent, which is what the
  target domain decides. Without ``fhr_st`` and ``fhr_ph`` in ``normalize_fields`` the target arrives
  at its stored scale, the Gaussian NLL is computed against a z-scale variance model, and the run
  trains a meaningless objective to completion with nothing raising; the guard that catches it lives
  in the shared entry point and reads the driver it was handed, so a ``trainer_cls=`` wiring mistake
  would leave it checking the *raw* model's field -- which these configs satisfy.
* ``compile_model_requested``, ``_build_model_kwargs``, ``create_model`` and
  ``_build_trainer_kwargs`` come from the **conv-Transformer** parent. The feature parent defines
  none of them, so lookup passes through, and ``torch.compile`` becomes permitted on a model whose
  feature-domain ancestor never exercised it. That is the right outcome -- it is the transformer
  encoder that makes compilation worth having -- but it arrives by resolution order rather than by
  anything written down, so ``tests/test_trainer.py`` asserts it explicitly. Shipped configs keep
...
#: Repository root: ``teb_vae/lag_attn_transformer_fs/trainer.py`` -> up three.
...
# `python -m teb_vae.lag_attn_transformer_fs.trainer` from the repo root sets __package__ and needs
...
from teb_vae.lag_attn_fs.trainer import LagAttnFsTrainer  # noqa: E402
...
from teb_vae.lag_attn_transformer_fs.task import SeqVaeLagAttnTrfFsTask  # noqa: E402
...
class LagAttnTrfFsTrainer(LagAttnFsTrainer, LagAttnTrfRwsTrainer):
    ...
    would take the feature side and each failure would be silent --
    ...
    * omit ``CHECKPOINT_STEM`` and it writes ``lag-attn-fs-*.ckpt``, interleaving two models'
...
def main(config_path: str) -> None:
    ...
    """
    run_training(config_path, trainer_cls=LagAttnTrfFsTrainer)
...
RUN_CONFIG: str | None = "teb_vae/lag_attn_transformer_fs/configs/default.yaml"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "teb_vae/lag_attn_transformer_fs/configs/default.yaml. Run from the repo root. Optional "

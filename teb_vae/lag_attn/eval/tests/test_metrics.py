...
def test_lag_to_seconds_undoes_the_datasets_up_shift() -> None:
    r"""With $\Delta_{UP} = -20$ s a peak at lag $\ell$ reads as $4(\ell + 5)$ s.

    Which is what the model documentation states, and the reason the subtraction is not a sign
    slip.
    """
    assert metrics.lag_to_seconds(0, up_shift_secs=-20.0) == pytest.approx(20.0)
    assert metrics.lag_to_seconds(10, up_shift_secs=-20.0) == pytest.approx(60.0)
...
def test_lag_to_seconds_maps_an_array_elementwise() -> None:
    ...
    assert seconds.tolist() == pytest.approx([20.0, 24.0, 28.0, 32.0])
...
def test_lag_seconds_physical_returns_a_float_column_from_a_tensor() -> None:
    ...
    assert seconds.tolist() == pytest.approx([20.0, 24.0, 28.0])

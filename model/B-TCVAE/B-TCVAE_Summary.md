...
$$\mathbb{E}_{p(n)} [\text{KL}(q(z|n) \| p(z))] = \underbrace{\text{KL}(q(z,n) \| q(z)p(n))}_{{1}-\text{ Index-Code MI}} + \underbrace{\text{KL}(q(z) \| \prod_j q(z_j))}_{{2}-\text{ Total Correlation}} + \underbrace{\sum_j \text{KL}(q(z_j) \| p(z_j))}_{{3}-\text{ Dimension-wise KL}}$$

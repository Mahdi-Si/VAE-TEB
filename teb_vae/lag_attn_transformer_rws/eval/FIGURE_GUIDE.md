...
Almost every figure here is drawn by the shared pipeline in `teb_vae/lag_attn_rws/eval`, and its entry says the same thing that pipeline's own guide says. The five under `encoder_attention/` are this package's, because the mechanism they draw exists only in this architecture.
...
A fourth applies to every figure that resolves a quantity **by cohort**, and it is the one an operator is most likely to assume rather than check.
...
**What this model is** is that same design with both history encoders replaced. Where the comparison architecture summarised each stream's history with a convolutional stem into a recurrent cell, this one uses a causal conv-Transformer: a dilated depthwise stem, then self-attention blocks with rotary position encoding — a **full causal prefix** on the FHR stream, and a **16-step sliding window** on the UP stream. Everything else is the same code: the objective, the geometry, the prior head, the lag cross-attention, the head-structured posterior, the shared horizon decoder. That is what makes the two comparable at all, and it is why only one directory of figures here is new.
...
**The likelihood percentage exists only under `gaussian_nll`.** Under `mse` a block score is a sum of squared errors rather than a log density, so exponentiating it yields no density ratio. An `mse` run therefore emits no such column, no row and no headline key, and records why in `results.coupling.pred_gap_percent.likelihood_space`. The two error-space percentages are unaffected: a squared error is a squared error under either likelihood.
...
| `kld_per_head` | $K_t$ split across the 4 **lag-attention** heads; sums back to the total | nats per anchor |
...
4. **A smaller KL is not a weaker coupling when the prior got stronger** — which is exactly what this architecture set out to do. The KL is $\mathrm{KL}(q \Vert p)$: improve $p$ and the divergence falls with no change in what the source contributed. Rank on `pred_gap`, and read the KL beside it as a description.
...
### Lag attention — where in the past the model looked
...
### Encoder self-attention — how each history state was built

**This is the other mechanism, and the one the `encoder_attention/` figures draw.** It has nothing to do with the lag axis above.

| Name | Plain meaning | Units |
|---|---|---|
| **stream** | `target` (the FHR history encoder) or `source` (the UP history encoder). Every table and figure here is resolved per stream. | — |
| **block** | which self-attention block of that stream's stack. The target has 4, the source 3, at the shipped geometry. | index |
| **head** $h$ | one of $H_e = 4$ encoder attention heads. Unrelated to the $M = 4$ lag-attention heads, which are a different mechanism entirely. | index |
| **temporal distance** $t - j$ | how far back inside its own stream a key sits from the anchor attending to it. $0$ is the anchor's own step. | steps (and seconds, $\times 4$) |
| `entropy_nats` | per head, how spread that block's attention is over its admitted keys, taken **per anchor and then averaged** | nats |
| `ceiling_attainable_nats` | $\operatorname{mean}_t \log\min(t{+}1, c)$, the highest entropy that block could have reached — $c = T$ on the target, $c = W_U$ on the source | nats |
| `entropy_ratio` | their quotient. **$1$ means uniform over everything admitted**; near $0$ means one key carries the whole row. The ratio is the readable quantity; the raw entropy is not. | dimensionless |
| `mass` | the share of a head's attention that landed at a given temporal distance, averaged over anchors and segments | probability |
| `median_distance_steps`, `p95_distance_steps` | mass-weighted quantiles of that distance profile, **per block** | steps |
| `composed_reach_median_steps`, `composed_reach_p95_steps` | the whole stack's measured reach, $\widehat R = \min(R_{\mathrm{conv}} + \sum_b \widehat d_b,\ T)$ | steps |
| `structural_bound_steps` | $R_U = \min(R_{\mathrm{conv}} + N_U(W_U - 1),\ T)$: what the source stack *could* reach. `null`, and `structural_bound_absent: true`, on the unbounded arm. | steps |
| `measured_reach_inside_lag_range` | whether the measured 95th-percentile reach is shorter than the furthest lag the cross-attention searches | boolean |

**Why the ceiling, and why per anchor.** At anchor $t$ only $\min(t+1, c)$ keys exist at all, so a head attending uniformly over everything available to it reads as increasingly *concentrated* the earlier the anchor when measured against a ceiling it could not reach — the same reason `attention` refuses $\log L$. And the entropy is taken per anchor and then averaged, never as the entropy of the averaged profile: a mixture's entropy is at least the mean of the entropies mixed, so the second reports a head whose focus *shifts* as one that has none.

**Why the reach is composed rather than measured directly.** A single block hops at most $W_U - 1$ steps; a stack of them composes, and the stem widens the base. $\widehat R$ uses each block's *measured* hop where the structural bound uses the largest hop it was allowed, so a stack whose every block put its mass at its window edge reproduces the bound exactly. The cap at $T$ is the structural formula's own: a stack that saturates it has reached the start of the segment, which is the most a reach can mean.
...
| **block** | one anchor's whole forecast: $H \times R = 30 \times 16 = 480$ raw samples. Also, in `encoder_attention/`, one self-attention block of an encoder stack — the two never appear on the same axis. |
...
| `all` | the pooled cohort, which `encoder_attention`'s tables carry as a label in the class column rather than as a separate file |
...
| $L$ | 91 | lag bins = ~6 minutes of UP history; the furthest searched lag is 90 steps = 360 s |
| $M$ | 4 | lag-attention heads |
| $H_e$ | 4 | encoder self-attention heads |
| $N_Y$ | 4 | target encoder attention blocks (full causal prefix) |
| $N_U$ | 3 | source encoder attention blocks |
| $W_U$ | 16 | source attention window, in steps = 64 s |
| $R_{\mathrm{conv}}$ | 21 | the depthwise stem's own reach, kernels $[5, 9]$ at dilations $[1, 2]$ |
| $R_U$ | 66 | the source encoder's structural receptive field, $= 264$ s — deliberately shorter than the 360 s lag search |
...
| `encoder_attention` | both streams' `encoder_entropy_ratio_*` and `encoder_attention_distance_*_steps` |

All of them are defined in **Terminology** above.

**Two figures under `encoder_attention/` wear this suffix and are not members of this family**, and the distinction matters when reading them. `encoder_attention_entropy_by_clinical_class.pdf` and `encoder_attention_distance_by_clinical_class.pdf` are drawn by that analysis itself, not by the runner's fan-out, because they resolve *fields* — per head, per block, per distance — that a violin cannot carry. They have their own entries below. The fan-out's own output for that analysis is `encoder_attention_per_recording_by_clinical_class.pdf`, which **is** a member of this family and holds the per-recording scalars in the table above.

**How they are misread.** Every violin holds one value per **recording**, not per segment, so a cohort with eight recordings is eight points however many segments they contributed — the width of a violin is not evidence. A cohort split that produces fewer than two groups emits **no figure at all** and records a skip, and the pooled output beside it is untouched. And a visible separation here is not a result: `cross_subgroup` is the analysis that says which separations survive being asked properly, and it tests a deliberately short list rather than everything drawn here.
...
**How it is misread.** The two shaded spans are **structural, not findings**. An unshaded version of this figure reads as a model that fails at both ends of every recording. A left-hand rise is also where a reader of *this* architecture is most likely to over-read: the target encoder's full causal prefix means an early anchor genuinely has less history to attend to, but so does every anchor of the comparison model, and the warm-up span is unscored in both.
...
**In plain terms.** *"By what percentage did reading the uterine pressure improve the forecast?"* The figure beside this one answers the same question in nats, which is the model's own unit and tells a reader nothing about proportion. This one answers it as a percentage, in the two ways a percentage can honestly be taken. The top two panels are about the **error**: how much of the heart-rate prediction error the pressure signal removed. The bottom panel is about the **likelihood**: how much more probable the observed trace became. **Zero is the null on all three**, and a negative value means the source made that recording's forecast worse.
...
- **dashed vertical at 0** — "no improvement".
- **shaded band** — the 95% bootstrap interval **on the mean**, over recordings. Not the spread of the data.
...
**How it is misread.** Four ways. The two spaces are on **separate axes on purpose** — a per-sample density ratio and an error reduction routinely differ by an order of magnitude. The likelihood percentage divides by the **fixed** 480-sample block rather than by each anchor's own scored-sample count, so it under-reports wherever forecast steps are masked; it is a floor, not an estimate — and an **empty bottom panel means the checkpoint was scored under `mse`**, where the quantity is undefined, not that the source added nothing. The middle panel's two violins are **not two results**. And none of these is `pred_gap` divided by a block score: that ratio is not defined here, and a reader reconstructing it from `d_base_mc_nats` will get a number that changes sign with its own denominator.
...
**How it is misread.** The x position is a rank, so "dimension 0" on this figure is the largest contributor and not latent coordinate $0$. The bars sum to the headline `source_conditioned_kl_raw`, which is what makes the spectrum a decomposition rather than a second quantity — and that total is only a *rate* while the prior variance is off its clamp. A **shorter** spectrum than a comparison run's is also not by itself a weaker source pathway: this architecture's whole claim is a stronger prior, and a stronger prior lowers every bar without the source having contributed less.
...
- **bottom, four curves** — `attention_profile_per_head`, one per **lag-attention** head. Head $m$ writes latent group $m$, which is what makes the split additive rather than an arbitrary slice.
...
- Related but not drawn here: `attention_entropy_nats` against `attention_entropy_attainable_nats`. Their ratio, not the raw entropy, is the readable quantity — the same convention `encoder_attention_entropy.pdf` uses on the other mechanism.

**How it is misread.** Three ways. The head-averaged curve is a mean over four distributions that need not agree. The support correction fixes each bin's *denominator* and cannot fix its numerator: attention rows are renormalised per anchor, so a truncated anchor pushes mass onto the short lags and no per-lag count knows it happened. And the shading is **structural**: it marks where the geometry, not the model, thins the evidence.

**The four heads here are not the four heads in `encoder_attention/`.** These write latent groups; those build history states. The counts coinciding at four is a coincidence of two independent config keys.
...
**What it shows.** One retained recording's lag attention as an anchor $\times$ lag field, head-averaged, with the warm-up boundary marked.

**Axes.** Time in segment (seconds) against compensated lag seconds, **lag increasing upward**; colour is the attention weight on a sequential scale, because the quantity is non-negative. The dashed vertical line is the warm-up boundary.

**Terms on this figure.**

- **the field** — `attn_weights` for one recording, averaged over the four lag-attention heads: rows are lags, columns are anchors.
...
**How it is misread.** It is **one recording**, drawn from the seeded stratified retention draw, not a representative case. Attention retention is opt-in (`eval_config.caps.attention`), so a run that did not ask for it emits no such figure at all; the absence is silence, not failure. The upper-left triangle is empty by construction rather than by measurement.

**It looks like `encoder_attention_heatmap.pdf` and is not the same object.** Both are triangular attention maps against time. Here the vertical axis is the **lag between the streams**; there it is the **key step inside one stream**. Reading one for the other is the single most likely misreading in this package.

## `encoder_attention/encoder_attention_entropy.pdf`

**In plain terms.** *"Is each encoder head reading one moment of its own history, or all of them at once?"* The two history encoders build their state by attending over their own stream's past. This asks how *concentrated* that attention is, head by head and block by block. A bar near **1** means that head spreads itself uniformly over everything it is allowed to see — it is summarising, not selecting. A bar near **0** means it has locked onto essentially one step. Neither is good or bad on its own; what makes the figure readable is that every bar is measured against the most spread-out it could possibly have been.

**What it shows.** Two panels, one per stream. Within each, one bar per (block, head) pair: the measured attention entropy divided by the highest entropy that block could reach, with the ceiling drawn as a line at $1$.

**Axes.** Block-and-head label across (`b0h0`, `b0h1`, …); the entropy ratio up, dimensionless.
...
**How it is misread.** The ratio is a shape statement, not a quality one: a head at $1.0$ has not failed, and a head at $0.05$ has not succeeded. What the figure supports is a *comparison* — between heads, between blocks, between arms of the window sweep, between cohorts on the class-resolved figure beside it. It also says nothing about **which** steps a head attends to, only how many; that is `encoder_attention_distance.pdf`. And the entropy behind every bar is taken per anchor and then averaged, never as the entropy of an averaged profile: a head whose focus *shifts* through the segment would otherwise read as a head with no focus at all.

## `encoder_attention/encoder_attention_entropy_by_clinical_class.pdf`

**In plain terms.** *"Does the encoder read history differently for the sick babies?"* The same entropy ratio as the figure above, averaged over blocks and heads, one bar per clinical class. If the acidosis bar sits well below the healthy one, that encoder is attending more narrowly on those recordings — which is a description of the mechanism, not a finding about the cohort.

**What it shows.** Two panels, one per stream. One bar per clinical class in ascending severity, each the block- and head-averaged `entropy_ratio` for that class, with the pooled value drawn as a dashed line.

**Axes.** Clinical class across, in the clinical order and palette described at the top of this file; entropy ratio up.

**Terms on this figure.**

- **bars** — the class-resolved rows of `encoder_attention_entropy.csv`, averaged over block and head.
- **dashed horizontal** — the pooled (`all`) value, so a class is read against the whole draw rather than only against its neighbours.
- **colours** — green / amber / red by severity, the same table every other cohort figure here reads.

**How it is misread.** **This is drawn by the analysis, not by the runner's violin fan-out, and it is not a violin.** Each bar is a single averaged number over the whole cohort's draw, not a distribution over that cohort's recordings — so it carries no spread, no interval and no $n$ per point. The per-recording distribution of the same quantity is in `encoder_attention_per_recording_by_clinical_class.pdf`, which *is* a violin and *is* the one to read for how much a cohort varies. And the draw behind this figure is capped by `eval_config.caps.encoder_attention` and stratified over shards, so a class with few segments in the draw is a thin measurement — `n_segments` in the CSV is the number to check before believing a difference. No test is computed here on purpose.

## `encoder_attention/encoder_attention_distance.pdf`

**In plain terms.** *"How far back does each head actually look?"* This is the figure the encoder replacement exists to be judged by. Every head's attention is collapsed onto one axis — how many seconds back from the anchor the key sat — and drawn as a profile. A curve that decays fast is a head reading the immediate past; a curve with weight far out is a head genuinely using long-range history, which is the thing the recurrent branch could not do on demand. On the source panels the dashed vertical is the window edge, and **there is nothing beyond it by construction**.

**What it shows.** One panel per (stream, block), four curves per panel — one per encoder head — of attention mass against temporal distance $t - j$, pooled rows only. The source panels carry the window bound as a dashed vertical.

**Axes.** Temporal distance in **seconds** ($4$ s per step) across; attention mass up, a probability that sums to $1$ across the axis.

**Terms on this figure.**

- **curves** — `mass` from `encoder_attention_distance.csv`, one per head.
- **x axis** — $t - j$, the distance *inside one stream*. $0$ is the anchor's own step. This is **not** the lag axis: see the third rule at the top of this file.
- **dashed vertical (source panels)** — $(W_U - 1) \times 4$ s, the furthest key a source block admits. Every bin past it is exactly zero, which is why the table stops there rather than writing out two thirds zeroes.
- **the target panels have no such line**, because the target encoder's prefix is the whole segment.

**How it is misread.** A wide profile is **not** a long reach. One block hops at most $W_U - 1$ steps on the source stream; what a *stack* reaches is those hops composed with the stem, which is the `composed_reach_*` figure in `encoder_attention_reach.csv` and in the headline — not the width of any one curve here. Conversely, the target panels are not evidence of unlimited memory either: the profile is what the encoder *used* on this capped draw, and a decaying target profile means it chose the recent past, not that it could not have chosen otherwise. And mass is averaged over **every** scored anchor while a bin at distance $d$ can only be written by the anchors $t \ge d$, so a far bin is divided by anchors that could not reach it and reads low for that reason alone. `n_contributing_anchors` in `encoder_attention_distance.csv` is the count that could write each bin: multiply by `n_anchors / n_contributing_anchors` for the availability-corrected profile. The uncorrected form is the one drawn and the one every quantile, mean distance and composed reach is taken over, so the two never disagree.

## `encoder_attention/encoder_attention_distance_by_clinical_class.pdf`

**In plain terms.** *"Does the encoder reach further back on the abnormal recordings?"* The same distance profile, averaged over blocks and heads, one curve per clinical class.

**What it shows.** Two panels, one per stream. One curve per clinical class, each the block- and head-averaged mass-by-distance profile for that class, with the source window bound marked.

**Axes.** Temporal distance in seconds across; attention mass up. Colours are the clinical palette.

**Terms on this figure.**

- **curves** — the class-resolved rows of `encoder_attention_distance.csv`, averaged over block and head. A class that measured nothing is absent rather than drawn flat, and the curves are recoloured by label rather than by position so an absent class cannot shift every colour after it onto the wrong one.
- **dashed vertical (source panel)** — the window bound, as above.

**How it is misread.** Averaging over blocks mixes stacks that reach different distances, so this figure supports "class A's encoder attention sits further back than class B's" and not "class A's encoder reaches $x$ seconds". For the reach, read `encoder_attention_reach.csv`. As with the entropy figure beside it, this is a **description on a capped draw**: no test, no interval, and `cross_subgroup` is where a cohort difference is adjudicated — on per-recording values, which these are not.

## `encoder_attention/encoder_attention_heatmap.pdf`

**In plain terms.** *"Show me one segment's encoder attention as it actually looked."* Every other figure in this directory is an average. This is one segment: time runs down the rows (which anchor is attending) and across the columns (which step it attended to), with brightness the weight. The empty upper-right triangle is causality — an anchor cannot read its own future. On the source panels a bright **diagonal band** hugging the leading edge is the sliding window doing exactly what it was configured to do; on the target panels, a bright vertical stripe means every anchor in the segment kept returning to one particular moment.

**What it shows.** One panel per (stream, block): a single segment's attention map from the capped draw, averaged over the encoder heads.

**Axes.** Key step $j$ in seconds across, anchor step $t$ in seconds down (origin at the top, so time runs downward on both axes); colour is the attention weight on a sequential scale.

**Terms on this figure.**

- **the field** — one retained $(T, T)$ map per block, head-averaged. Head-averaged because the panel is about *when* rather than *which head*: the per-head resolution is in the profile figure, where four curves are readable and four heatmaps are not.
- **empty upper-right triangle** — structural: key $j > t$ is masked, exactly zero, never measured.
- **empty lower-left region on the source panels** — also structural: a source block admits no key beyond $W_U - 1$ steps back, so only a narrow band along the diagonal is ever non-zero.
- **`interpolation='none'`** — cells are drawn as cells. This is a vector output whose reader is expected to index one, and a resampled cell boundary can land half a cell from where the data says it is.

**How it is misread.** It is **one segment of the capped draw**, chosen by the pass rather than by any property of the data, and it is an illustration rather than evidence — the same standing the per-sample pages have. The two triangles are masks and not measurements. And the axis is **not** the lag axis of `attention/lag_heatmap.pdf`: there, the vertical axis is the delay between UP and FHR; here, both axes are steps of one stream and the interesting structure is the distance from the diagonal.
...
**How it is misread.** The dashed lines, not the dotted ones, are what "pinned" means. The bound is a sigmoid, so the asymptote is never reached and mass exactly *at* a clamp would be invisible. The two ends also fail in opposite directions: mass on the floor is an over-confident decoder whose squared NLL term explodes, mass on the ceiling is one that has given up. **The prior's variance is not on this figure** — it is in `latent/`, and it is the one whose clamp inflates every coupling number.
...
**Axes.** Frequency in Hz on a **log** scale, from $\Delta f = 7.8$ mHz to $2$ Hz; lead time in seconds, $0$–$120$; colour is coherence (top, unsigned) and a signed coherence difference (bottom, symmetric about $0$). The scale is logarithmic because the bands this analysis exists to report are crowded into the bottom of the range. **The DC bin is not drawn**: a log axis has no room for $f = 0$, and that bin is the Hann taper's residue of the removed per-window mean rather than a frequency. It is not lost — it is part of the `vlf` band sum in every other coherence output.
...
**How it is misread.** **The truth's line is the control and must be read first.** A ratio above $1$ on a branch means nothing on its own — the fetal heart rate has its own content at $0.25$ Hz. Only the *excess over the truth* is an artifact. If there is one, it lands inside the `mf` band, so `mf`'s coherence and gain in every other coherence figure are contaminated. This is a property of the **decoder**, which is shared unchanged with the comparison model — so a seam artifact is not evidence about the encoder replacement, and no verdict fails on it.
...
| `attention_entropy_nats` | how spread the **lag** attention was — not the encoder's, which is in `encoder_attention/` |
...
**Why it is one readout.** The bottom panel is a single axis in nats per anchor, and $K_t$ on it beside `pred_gap` is routinely orders of magnitude larger — a shared page draws the gap as a flat line along the bottom of the KL's range. $K_t$ has its own page beside this one.

## `trajectory/trajectory_profile_kl.pdf`

The same two views of `kld_per_t` ($K_t$, how far UP moved the belief at each anchor), on its own axis. Read as the page above, with the KL's one caveat: it is **inflated by an arbitrary factor whenever the prior variance sits on its clamp**, so its level is not comparable across checkpoints. `pred_gap` carries no such factor, which is why both are reported and neither is read alone.
...
- **one line per class** — `hie` (red), `acidosis` (amber), `healthy` (green), in that order: cohorts are read worst first everywhere in this evaluation, which is also what orients every significance test.
- **line** — median over recordings in that window; **ribbon** — inter-quartile range over recordings. Each recording contributes one value per window, averaged over its own segments in that window.
...
**How it is misread.** The annotated $n$ is the point of the figure as much as the line is: a window's median can move because the cohort changed rather than because the coupling did. The classes do not cover the axis equally, so comparing two lines at a window where one has three recordings and the other forty is comparing a number against noise.

**Why it is one readout.** The unfloored KL shares the nominal unit and not the scale, so a page carrying both puts `pred_gap` on a range the KL set. It has its own page beside this one, and the two are compared in `time_to_delivery_trajectory.csv` rather than by eye.
...
**Axes.** Time before delivery in hours on the same $0.5$ h grid as the trajectory, inverted so delivery is at the right — on every panel including the heatmap; nats per anchor on the violins; $-\log_{10} p$ on the strips.

**Terms on this figure.**

- **violin body** — the kernel density of that cell's per-recording values; **heavy bar inside it** — $Q_1$ to $Q_3$; **hairline** — Tukey's adjacent values; **white dot** — the median.
- **dots instead of a body** — a cell with fewer than `MIN_GROUP_SIZE` = 3 recordings, or one whose values are all equal. Those are also the cells the test excludes, so a cell drawn as dots carries no verdict either.
...
**A bar that is absent and a cross at zero are different statements.** No bar with no cross means the window was tested and its $p$ came out at or near 1; a cross means fewer than two classes had enough recordings there, and nothing was tested.

**A window is not an independent experiment.** The correction is across the windows of this clock as one family. The two readouts' pages are **not** jointly corrected, because they are two readings of the same recordings rather than two hypotheses.

## `time_to_delivery/time_to_delivery_windows_kl.pdf`

The same three panels for `source_conditioned_kl_raw`. Read exactly as the page above — including the row order and the sign of Cliff's delta — remembering that the KL's *level* is inflated wherever the prior variance is clamped.

## `second_stage/second_stage_trajectory_pred_gap.pdf`

**In plain terms.** *"Does the predictive gain change around the moment the second stage of labour begins?"* The same readout as the delivery clock, on the other clinical landmark — the one inside labour rather than at its end. Only the recordings the labour-onset table places a second stage for appear on it.
...
- **line** — median over recordings in that window; **ribbon** — inter-quartile range over recordings.
- **annotated number at each point** — `n_recordings` in that class-and-window cell.
...
**How it is misread.** **The sign is the opposite convention from the delivery clock's**: negative is *before* the onset. **The positive side is short by construction** — the second stage begins a couple of hours before delivery, so windows after onset hold far fewer recordings than windows before it. And the population is a **subset**: a recording with no recorded onset is dropped and counted in `second_stage_eligibility.csv`.

## `second_stage/second_stage_trajectory_kl.pdf`

The same figure for `source_conditioned_kl_raw` on this clock, on its own axis. Read as the page above, with the KL's clamp caveat: its level is inflated wherever the prior variance sits on its clamp, so the shape is readable and the height is not.

## `second_stage/second_stage_windows_pred_gap.pdf`

**In plain terms.** *"That second-stage trajectory has three lines on it — is the gap between them real, and where?"* The same page `time_to_delivery/time_to_delivery_windows_pred_gap.pdf` draws, on the other landmark.

**What it shows.** Three panels of `mc_pred_gap`. A violin per (window, clinical class) cell over one value per **recording**; beneath it $-\log_{10}$ of that window's Holm-adjusted $p$ against the $\alpha$ line; then a heatmap of Cliff's delta for every class pair that survived Holm, in any window.

**Axes.** Signed hours from second-stage onset on the same $0.5$ h grid as the trajectory, negative before onset and positive after, **not** inverted and with the onset marked at zero; nats per anchor on the violins; $-\log_{10} p$ on the strips.

**How it is misread.** **This clock's Holm family is its own** and is *not* joint with the delivery clock's, because the two are different alignments of an overlapping population — so a window significant on one and not the other is a statement about alignment, and a reader quoting a claim from both clocks is making two comparisons.
...
**A bar that is absent and a cross at zero are different statements.** No bar with no cross means the window was tested and its $p$ came out at or near 1; a cross means fewer than two classes had enough recordings there — which on the positive side of this axis is the common case rather than the exception.
...
**Axes.** Top: $-\log_{10} p$, one bar per metric. Bottom: metrics down, cohort pairs across, colour symmetric about zero. The pair columns are ordered by where their two cohorts fall in the clinical order.

**Terms on this figure.**

- **top bars** — $-\log_{10}$ of `p_holm`, the Holm-adjusted $p$-value of a **Kruskal-Wallis** omnibus test per metric. Floored so that $p = 0$ does not become an infinite bar.
- **dashed vertical** — $\alpha = 0.05$, Holm-adjusted. Bars to its right survived.
- **Holm** — a step-down correction across the metrics **as one family**, so testing eight things does not manufacture a significant one.
- **bottom cells** — **Cliff's delta** for a pair of cohorts: $P(X > Y) - P(X < Y)$, running from $-1$ to $+1$. Magnitude labels follow Romano: $<0.147$ negligible, $<0.330$ small, $<0.474$ medium, above that large.
...
- Every test consumes **one value per recording**.

**How it is misread.** The two panels answer different questions and only the second is about size. The lower panel is **empty by design** when nothing survived Holm — that is a result, not a missing figure. Every test behind it consumes one value per **recording**, so a bar's height reflects tens of observations rather than the thousands of segments they were reduced from, and a signed delta's direction is only interpretable next to the `higher_is_better` column in `cross_subgroup_significance.csv`.

And the `<left> vs <right>` naming inside each column runs the same way the column ordering does: which cohort of a pair is `left` comes from the shared pairwise test, which names a pair in the order it receives the cohorts and receives them worst first. So a column reads `hie vs acidosis` in the HIE-first position, and a positive delta means the more severe cohort runs higher — read the label, not the position, for the sign all the same.
...
- **hit rate** — of the true decelerations at this lead time, the fraction the forecast also contained.
...
- Rates are **per event**, not per anchor — fixing the horizon step is what makes that exact.
- A hit rate of **zero** on a checkpoint that has not learned to forecast is the expected reading, not a broken detector. `n_forecast_events` tells the two apart.

**How it is misread.** The horizontal axis does **not** span the whole two-minute horizon, and the part it omits is not missing data. Every rate is **per event**, not per anchor: consecutive anchors' blocks overlap in 29 of their 30 steps, so an anchor-level rate would count one physiological deceleration once per anchor. The `pseudo_replication_factor` in the JSON is that ratio, measured. And a hit rate of zero on a checkpoint that has not learned to forecast is the expected reading rather than a detector failure.
...
- **guards** — below 200 event anchors over 4 recordings the analysis draws nothing and records a skip.

**How it is misread.** Zero is the null here, not the value. Controls are drawn **within** each recording and matched to that recording's event count, so this is not a comparison between recordings wearing two labels — but it is still observational: an anchor near a contraction is also an anchor at a particular point in labour. The guards matter as much as the number.
...
**What it shows.** Two panels. The upper one is three violins over the **held-out** recordings — $D_{\mathrm{oracle}}$, $D_{\mathrm{base}}$ and $D_{\mathrm{full}}$ — with both gaps annotated in the title. The lower one is the probe's own fit: held-out and fit score against optimizer step, for the comparison probe and for the doubled-width refit the capacity check uses.
...
**And `target_state` here is a conv-Transformer state, not a recurrent one.** The first bias is therefore a different quantity than it is for the comparison model, so the two architectures' sufficiency gaps are comparable only with that stated.

**The lower panel is a precondition, not a decoration.** A held-out curve still descending at its right-hand edge is a probe that has not finished, and an unfinished probe understates the gap. Read the upper panel only once the lower one has flattened.

**Only the held-out recordings are drawn.** $D_{\mathrm{base}}$ here is *not* the run's headline `d_base_mc_nats`.

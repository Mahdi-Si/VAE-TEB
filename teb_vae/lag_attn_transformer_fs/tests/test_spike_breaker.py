...
so it goes negative harder and earlier than either raw model. The breaker's relative test is
...
Ported rather than inherited, and the port earns its place twice over. Every threshold in that block
is stated in nats of the summed block, so none of the conv-Transformer raw sibling's values transfer
and none of its tests are evidence about this configuration. And the one threshold this package
*could* have inherited -- the conv-LSTM feature model's margin, measured at this same block
cardinality -- was re-derived here rather than carried, because a change of encoder changes the
gradient population and could have changed the loss's epoch-to-epoch fluctuation with it. It did not:
the shipped margin sits at the same multiple of this stack's own movement as it did of that one's,
which is a measurement and is asserted below as one.

Each test drives the breaker with the block **this** ``configs/default.yaml`` ships (warm-up
shortened to zero so the gate is active), at magnitudes this objective actually reaches, so a config
...
#: The two comparison configs, one per axis of the grid: the same encoders at the raw target, and the
#: same target through the conv-LSTM encoders.
_TARGET_SIBLING_CONFIG = (
    _REPO_ROOT / "teb_vae" / "lag_attn_transformer_rws" / "configs" / "default.yaml"
)
_ENCODER_SIBLING_CONFIG = _REPO_ROOT / "teb_vae" / "lag_attn_fs" / "configs" / "default.yaml"

#: A loss magnitude this objective genuinely reaches, used to settle a healthy negative EMA. Chosen
#: at the scale of the summed block: the instrumented run's post-warm-up ``main_loss`` sat at
#: $4880 \pm 436$ in magnitude, and the sign turns over as the decoder's log-variance falls. A test
#: that settled its EMA at $-500$ would be testing a raw model's regime under this config's
#: thresholds.
...
# --------------------------------------------------------------------------------------
# Where this block's thresholds sit in the grid, and why
# --------------------------------------------------------------------------------------
def test_the_margin_is_the_block_s_and_not_the_encoder_s():
    """The margin lives on the target axis, which is the claim the two comparisons make together.

    It is *larger* than the conv-Transformer raw model's because that block is $480$ samples against
    $2340$ coefficients and the margin is stated in nats of the summed block. It is *equal* to the
    conv-LSTM feature model's because that block is the same one -- and equal by measurement rather
    than by inheritance: the instrumented run at this encoder put the shipped value at $4.0\\times$
    the largest epoch-to-epoch ``main_loss`` movement it observed, against $3.3\\times$ for the value's
    own derivation at the other encoder.
    """
    mine = load_config(str(_CONFIG))["advanced_config"]["spike_breaker"]
    target_axis = load_config(str(_TARGET_SIBLING_CONFIG))["advanced_config"]["spike_breaker"]
    encoder_axis = load_config(str(_ENCODER_SIBLING_CONFIG))["advanced_config"]["spike_breaker"]

    assert mine["additive_margin"] > target_axis["additive_margin"]
    assert mine["additive_margin"] == encoder_axis["additive_margin"]


def test_the_floor_and_the_shape_of_the_block_are_the_family_s():
    """Everything except the margin is shared with both comparison models, and the ``ema_floor`` is
    the one worth stating: it is not a scale at all but a switch that turns the relative test off by
    ...
    for other in (target_axis, encoder_axis):
        assert mine["ema_floor"] == other["ema_floor"] >= 1.0e9
        assert mine["multiplier"] == other["multiplier"]
        assert mine["max_consecutive_skips"] == other["max_consecutive_skips"]
        assert mine["comparison_metric"] == other["comparison_metric"]


def test_the_floor_still_exceeds_any_loss_this_objective_can_reach():
    r"""Confirmed rather than assumed, because the block that has to stay under it is the largest in
    the family. The reconstruction is summed over $2340$ coefficients and the per-coefficient
...
def test_the_shipped_margin_catches_a_finite_blowup(task):
    """A finite jump with no NaN anywhere, re-enacted at this objective's scale. The non-finite guard
    has nothing to catch; the additive test is the one that must fire, against the raw (negative)
    EMA."""
    module = task()
    config = _shipped_breaker()
    for _ in range(5):
...
def test_the_margin_leaves_the_observed_epoch_to_epoch_movement_alone(task):
    ...
    Driven at the movement the instrumented run actually observed rather than at a fraction of the
    margin: over its $100$ post-warm-up epochs the largest epoch-to-epoch $|\\Delta|$ of ``main_loss``
    was $1258$ nats, and that number is what the shipped margin was set against. A margin re-derived
    downwards to inside it would fail here.
    """
    module = task()
    config = _shipped_breaker()
    largest_observed_move = 1258.0
    for _ in range(5):
        _feed(module, _HEALTHY_LOSS, config)

    metrics, _ = _feed(module, module._spike_ema_loss + largest_observed_move, config)

    assert not _skipped(metrics)
    assert float(config["additive_margin"]) > 3.0 * largest_observed_move
...
def test_the_configured_comparison_metric_is_one_the_task_emits(
    ...
    missing, so the config must name something the task genuinely emits -- and the metric set here
    reaches the task through two levels of inheritance."""
    config = _shipped_breaker()
    module = task()
...
def test_a_skipped_step_still_touches_every_parameter(task):
    ...
    # A non-finite loss whose autograd graph spans every trainable parameter, as the real loss does;
    # a leaf NaN would prove nothing about the hooks.

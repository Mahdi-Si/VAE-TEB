...
**The interactive page.** Beside the PDF, `<guid>_<subgroup>_trace.html` is the same trace as a plotly page (plotly.js from its CDN): the raw FHR and UA the loader served on top, then every row below, on one shared zoomable time axis relative to delivery. Every series is painted onto a uniform grid in `epoch` order, so where a dataset stores overlapping segments the **later segment overwrites the earlier one** and each instant carries the latest reading; a break is an empty stretch. The lag map and, on the lag-slot cell, the bounded mean update are on logarithmic colour (a signed log for the update), ticked in the panel's own units.
...
**What it shows.** One row per class, five columns. First the **inputs**: the declared target stream (the scattering block above the phase-harmonic block, divided by a rule) and the declared source stream, standardised coefficients over stored second and declared channel on a sequential scale with robust (1st–99th percentile) limits. **The cells the model never read are blanked** in light grey rather than drawn: every channel's cold steps, which the input gate multiplies by zero, so the warm-up region neither sets the colour scale nor reads as data. Then the attribution of $K_t$: to the target stream under the **all-zero** baseline — the only baseline along which the target inputs move, so the only one under which a target map exists — and to the source stream under the **source-null** baseline, the primary comparison, each on a **symmetric-log** colour scale (linear within four decades of the map's largest magnitude, logarithmic beyond) with the cold cells and every step after the anchor blanked, since both are exactly zero by construction. Last, the source attribution re-indexed by offset from the anchor, as a share per lag on a symmetric-log axis, against the model's own lag readout at the same anchor normalised the same way, with their correlation and Jensen–Shannon distance in the title. Every map carries the anchor as a vertical rule and the forecast block it scores as a shaded window; the row label carries the GUID, the subgroup, the class, the anchor step and its hours before delivery.

**Axes.** Stored time in seconds across (the samples pages' axis); declared channel down; standardised coefficient (inputs) or readout units on a symmetric-log scale (attribution) in colour. The lag panel is in stored-coefficient seconds.

**Interpretation.** Four ways. *Every cell after the anchor is exactly zero* on a causal model and is blanked, not cropped — `attribution_checks.pdf` is where a non-zero there would show, as a defect rather than a finding. *The map is one anchor of one segment of one recording*, drawn for looking at; the population reductions are the other figures, and the per-class example pages below give the same anchor under every readout. *The colour scale is per panel*, so two panels cannot be compared by colour, only by the totals in `attribution_rows.csv`. *A cold channel's cells are zero because the gate multiplied them by zero*, not because the model found nothing there. The caveat under the figure applies to every lag panel.
...
**What it shows.** One page per class example, laid out as the samples pages are: a stack of full-width rows on **one shared stored-time axis**, with a colour-axis column on the right, so a column of the page is the same stored second on every row and maps are compared by reading down. The first two rows are the input streams the encoders read, cold cells blanked. Every readout variant then takes two rows — its target attribution under the all-zero baseline above its source attribution under the source-null baseline, each on a symmetric-log colour scale with the readout's value at the input and at the exact null in the row title. The variants, in order: the divergence $K_t$, the forecast gap (base minus full), the full-branch block score, the full-branch **squared error** (the fidelity of the mean forecast, which the learned variance cannot trade against), the squared-error gap, the full-branch score at the **first and the last horizon step**, then the lag readout on each configured `occlusion_bands` band. Three rows off the time axis close the page: the **latent at the anchor** — prior mean, source shift $\mu^q - \mu^p$ and per-coordinate divergence $K_{t,d}$, with the top coordinate marked; the **activation split** — the layer integrated gradients of every variant on the cell's own layer (per head on the attentive cells, per lag on the lag-residual cell), source-null baseline; and every variant's lag-aligned source attribution overlaid on one lag axis against the model's lag readout. The page title carries the GUID, the subgroup, the class, the anchor step and its hours before delivery.

**Axes.** Stored time in seconds across on every map row, declared channel down; latent coordinate, head or lag, and stored-coefficient seconds on the three closing rows.

**Interpretation.** Read down a column to see whether the same coefficients drive different readouts, and across a row to see where in stored time and in which channels one readout was sensitive. The sign matters and differs by readout: a positive cell raises the readout along the path, which is a larger latent change for $K_t$, a larger advantage of full over base for the gap, and a *worse* forecast for the block score. The all-zero and source-null columns are attributions along different paths from different baselines and do not sum to anything together; the lag-band readout's own attribution is expected to concentrate on its band, and how far it does is what the last rows show. The two horizon-step rows say whether the near and the far end of the block read the inputs differently; the squared-error rows say which inputs moved the forecast itself rather than its score. `attribution_maps.npz` carries every map on the pages, keyed by example, readout, baseline and band (`h<step>` for a horizon step), beside the inputs, the latent (`example_latent_*`) and the activation split (`map_layer`).

## `attribution/attribution_blocks.pdf`

**Purpose.** Answer, per readout, which of the four input blocks the model responded to — the question behind the NLL and the forecast-fidelity analysis: does the score, the error, the latent change and the gain come from the target's scattering coefficients, its phase harmonics, the source's scattering coefficients or its phase harmonics.

**What it shows.** Left: per readout variant — the divergence, the forecast gap, the full-branch block score, the full-branch squared error and the score at the first and the last horizon step — the mean over recordings of the **unsigned** attribution's share in each block under the all-zero baseline, the one path along which every stream moves, as a stacked bar summing to one. Right: the same rows' **signed** block sums on a symmetric-log axis, so a block that raised a readout and one that lowered it are told apart. `attribution_blocks.csv` carries both under both baselines.

**Axes.** Share of the unsigned attribution across (left); signed attribution in readout units, symmetric log (right); readout variants down.

**Interpretation.** A share is a split of sensitivity along one path from one baseline, not a variance decomposition. The block-score and squared-error rows read against each other: a block that carries the score but not the error is one that moved the forecast's variance rather than its mean. Under the all-zero baseline the source's share includes what the source contributes when introduced together with the target, which is not the source-null comparison the lag figures use.

## `attribution/attribution_horizon.pdf`

**Purpose.** Show how the near and the far end of the forecast block read the inputs.

**What it shows.** Over the per-step score rows (`nll_horizon`, tagged `h<step>` in the `band` column). Left: per named horizon step, the mean over recordings of the unsigned attribution total of the target stream (all-zero baseline) and of the source stream (source-null baseline), with the step's score at the input as a line. Middle: the source attribution by lag per step, source-null baseline, magnitude. Right: the target attribution by offset from the anchor per step, all-zero baseline. All vertical axes symmetric-log.

**Axes.** Horizon step across (left); offset from the anchor in stored-coefficient seconds across (middle, right); nats up.

**Interpretation.** A far step that reads the source more than the near one is a source whose information pays later in the block; a lag profile that shifts between the steps is a model reading further back for further-out forecasts. The steps are the first and the last of the block, so this is the two ends and not the shape between them. The axis is stored-coefficient time.
...
**Axes.** Declared channel across; attribution in readout units up, symmetric log.
...
**Axes.** Offset from the anchor in stored-coefficient seconds across; declared channel down; mean unsigned attribution in colour on a logarithmic scale, one scale per panel.
...
**Axes.** Offset in stored-coefficient seconds across; attribution in readout units up, symmetric log.

...
**Three things differ from the raw sibling's oracle, and all three are the target domain.** The
probe emits one value per surviving target *channel* per horizon step rather than $R$ raw samples;
the forecast block is gathered by the model's own :meth:`_build_forecast_target` at the anchor set
the forward returned, rather than by ``build_future_target`` over a dense raw index grid; and the
mask is the anchored :func:`~teb_vae.lag_attn_rws.nets.raw_masks.forecast_mask`, so the probe is
scored over exactly the anchors the collection pass scored the two model branches over.
``model.future_index`` is not read at all here -- it indexes a raw grid this target does not have.
...
makes the oracle look better than it is and the gap larger than it is, and a GUID contributes
tens of overlapping segments, so a segment-level split is not a split at all. The GUIDs are
...
**The forecast target is rebuilt, never cached.** The stored feature stream is $(T, c_y)$ against
the $(A_{\max}, H, C_{\mathrm{keep}})$ block it generates -- $120$ KiB against $873$ KiB at the
shipped geometry, a factor of seven -- and it is pushed through the *model's own*
:meth:`_build_forecast_target` and the anchored :func:`forecast_mask` at the model's own
``coverage_floor``, so the oracle is scored on the same anchors, against the same coefficients, as
the branch it is compared with.
...
#: *every* segment, and the cap exists only to bound the cache on a split large enough for a
#: quarter of a megabyte per segment to matter.
...
#: Ceiling on the largest tensor one fit step stores, in elements -- and the reason the batch above
#: is a *starting* value here rather than the setting.
#:
#: **This is the one budget the raw pipeline did not need.** The horizon core keeps a
...
#:
#: Four million holds that peak near a gigabyte at the shipped geometry, which is the measurement
#: this number comes from rather than a round figure. Lowering the batch costs nothing but
#: optimizer steps: the budget below is expressed in *passes over the fit half*, so the step count
...
#:
#: The ceiling is stated as :data:`MAX_FIT_STEPS` steps **at the default batch** and applied as a
#: bound on segment-visits, so that a batch lowered by the activation ceiling above buys more steps
#: rather than fewer passes. A ceiling on iterations alone would silently convert a memory bound
#: into an under-trained probe, which is the one failure this module is built to report rather than
#: absorb.
MIN_FIT_STEPS = 4
MAX_FIT_STEPS = 3000
MAX_FIT_SEGMENT_VISITS = MAX_FIT_STEPS * DEFAULT_FIT_BATCH_SIZE
...
BIAS_DIRECTIONS: Tuple[Dict[str, str], ...] = (
    {
        ...
        "cause": (
            "the probe conditions on target_state rather than on the target's own coefficient "
            "history, so it inherits the encoder's own information loss and cannot recover what "
            "the encoder already discarded -- the measured gap is therefore the bottleneck's cost "
            "alone, not the whole context-sufficiency gap"
        ),
    },
    {
...
#: What a block score here is summed over, stated in the record because it is not the sibling's.
#: A nats-per-anchor figure from this cell is a sum over $H \cdot C_{\mathrm{keep}}$ coefficients
#: whose $C_{\mathrm{keep}}$ the warm-up budget decides, so it is comparable neither with the raw
#: cells' nor across budgets of this one.
BLOCK_CONVENTION = (
    "a block score is summed over horizon x surviving-target-channel coefficients and averaged "
    "over contributing anchors, so it is budget-local: two arms at two warm-up budgets sum over "
    "different numbers of channels and their nats are not comparable"
)
...
def build_probe(model: Any, *, width_multiplier: int = 1) -> nn.Module:
    ...
    shared horizon core and emits $(B, A, H, C)$ over whatever anchor axis it is handed, so the
    oracle is that class built at the encoder state's width. Everything that decides *capacity* --
    the hidden width, the core's depth, its kernel, its FiLM arrangement, its horizon-attention
    depth, the log-variance clamp -- is read off the model that is being measured rather than
    restated, which is what makes "the comparison isolates the bottleneck" a property rather than
    a claim.

    ``out_channels`` is the production decoder's own, which on this cell is $C_{\mathrm{keep}}$ --
    the surviving-channel count the warm-up budget resolved. Read off the built decoder rather
    than recomputed from the gate, so a probe cannot emit a width the target does not have.
    ...
    * the output heads are calibrated onto the trivial $\mu = 0, \sigma = 1$ predictor whatever the
    ...
    Args:
        model: The loaded :class:`~teb_vae.lag_attn_cfs.nets.model.SeqVaeLagAttnCfs`, or the
            transformer cell's equivalent.
...
class StateCache:
    ...
    Attributes:
        ...
        target_features: The loader-normalized target stream $(N, T, c_y)$, at the **declared**
            width the model's keep-index is positional into. The expanded forecast block is
            rebuilt from it per batch rather than cached: it is a seventh of the size and is
            produced by the model's own gather.
        weight: The decimated validity signal $(N, T)$, from which the forecast mask is rebuilt.
        anchor_index: The anchors the forward decoded $(N, A_{\\max})$, ``long``. Cached rather
            than rederived for the reason the forward returns it at all: a second computation
            could disagree, and the disagreement would be a wrong number rather than an exception.
        anchor_valid: Which of those anchors are real $(N, A_{\\max})$, ``bool``. At the dense
            geometry every entry is real, and caching it anyway is what keeps this correct on an
            arm whose floor and horizon leave a padded slot.
    ...
    def n_bytes(self) -> int:
        """Bytes held by the five tensors, so the amendment to the one-pass rule is a number."""
        return int(
            sum(
    ...
    def describe(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "anchors_per_sample": int(self.anchor_index.shape[1]) if len(self) else 0,
        }
...
def cache_target_states(
    ...
    branches -- but the target stream reaches the encoder through a channel gate, a warm-up
    availability adapter and a delay, and a state built by a second copy of that sequence is a
    state the production decoder never saw. The cost is one pass at roughly an eighth of the
    collection pass's, since that one decodes four branches at $K$ draws and this one decodes two
    at one.

    The forward is called at :data:`~teb_vae.lag_attn_cfs.eval.metrics.DENSE_ANCHOR_GEOMETRY`, the
    same $(\varphi, S) = (0, 1)$ the collection pass used, so the anchors the probe is scored over
    are the anchors ``nll_base_block`` was measured over. A pass at the training tiling would
    report a different population under the same name.
    ...
    for batch in pages:
        ...
        features.append(target_features.detach().to("cpu", torch.float32))
        weights.append(weight.detach().to("cpu", torch.float32))
        anchors.append(outputs["anchor_index"].detach().to("cpu", torch.long))
        valids.append(outputs["anchor_valid"].detach().to("cpu", torch.bool))
    ...
    if not states:
        return StateCache(
            ...
            target_features=torch.zeros((0, int(model.geometry.t), int(model.c_y))),
            weight=torch.zeros((0, int(model.geometry.t))),
            anchor_index=torch.zeros((0, 0), dtype=torch.long),
            anchor_valid=torch.zeros((0, 0), dtype=torch.bool),
            guid=[],
    ...
    cache = StateCache(
        ...
        anchor_index=torch.cat(anchors, dim=0),
        anchor_valid=torch.cat(valids, dim=0),
        guid=guids,
        epoch=np.asarray(epochs, dtype=np.float64),
    )
    logger.info(
        f"oracle: cached {len(cache)} segment(s) from {len(set(cache.guid))} recording(s), "
        f"{cache.n_bytes / (1024 ** 2):.1f} MiB of encoder state and target stream"
    )
    return cache
...
def guid_split(
    ...
    delivery contributes tens of segments whose forecast windows overlap in $H - 1$ of their $H$
    steps, so a segment-level split leaves the probe scoring what it was fitted on under another
    name.
...
def _batch_tensors(
    ...
    *,
    ...
    r"""Assemble one batch of ``(states at the anchors, target, mask)`` from the cache.
    ...
    **The anchor set is the target row's**, in both cases. It is a deterministic function of the
    geometry at the dense $(\varphi, S) = (0, 1)$ and therefore identical on every row, so the
    choice is inert today; it is written this way because the target and the mask are built from
    it, and a control that gathered a stranger's *anchors* would score one segment's coefficients
    against another segment's window.

    Under ``no_grad`` in its entirety: everything here is data. Nothing on this path is a function
    of a model parameter, and building it inside the graph would be the one route by which a probe
    step could reach the checkpoint.

    Args:
        model: The loaded net, for its own target gather, its geometry and its coverage floor.
        cache: The cached pass.
        target_rows: Cache positions supplying the forecast target, its anchors and its mask.
    ...
    Returns:
        ``(states, target, mask)``: $(B, A_{\max}, d_{\mathrm{model}})$,
        $(B, A_{\max}, H, C_{\mathrm{keep}})$ and $(B, A_{\max}, H)$.
    """
    states = cache.target_state[state_rows].to(device=device, dtype=torch.float32)
    features = cache.target_features[target_rows].to(device=device, dtype=torch.float32)
    weight = cache.weight[target_rows].to(device=device, dtype=torch.float32)
    anchors = cache.anchor_index[target_rows].to(device=device)
    ...
    # The conditioning, gathered to the same anchor axis the forecast is emitted on -- which is
    # what the production forward does with ``z`` before handing it to this same decoder class.
    gathered = states.gather(
        1, anchors[:, :, None].expand(-1, -1, int(states.shape[-1]))
    )
    return gathered, target, mask
...
def score_rows(
    ...
    *,
    ...
    Args:
        ...
        model: The loaded net, for the target gather, the geometry and the coverage floor.
    ...
    with torch.enable_grad():
        for step in range(1, steps + 1):
            ...
            states, target, mask = _batch_tensors(
                model, cache, rows[picks], state_source[picks], device=device
            )
            mu, logvar = probe(states)
            block, contributing = masked_raw_block_per_anchor(
                mu, target, mask, likelihood=likelihood, logvar=logvar
...
def activation_elements_per_segment(model: Any, *, width_multiplier: int = 1) -> int:
    r"""Return the elements one segment contributes to a probe's largest stored activation.

    $A_{\max} \cdot H \cdot d_{\mathrm{hidden}}$: the shape the horizon core keeps alive per refine
    block and per attention block for the backward pass. Read off the model rather than from a
    config, for the reason :func:`build_probe` reads everything else there.

    Args:
        model: The loaded net.
        width_multiplier: The probe width the bound must hold for. Pass
            :data:`CAPACITY_WIDTH_MULTIPLIER` when the capacity refit will run, so **one** batch
            size is safe for both fits -- which is also what keeps the two probes differing in
            width and nothing else a step count could account for.
    ...
    return int(
        max(1, anchors)
        * int(model.horizon)
        * int(model.horizon_core.d_hidden)
...
def resolve_budget(
    ...
    *,
    ...
    activation_per_segment: Optional[int] = None,
) -> Dict[str, Any]:
    ...
    Args:
        ...
        batch_size: Segments per step, or ``None`` for :data:`DEFAULT_FIT_BATCH_SIZE`. Clamped to
            ...
            pays for each one. Clamped again by ``activation_per_segment``.
        ...
        activation_per_segment: Elements one segment adds to the largest stored activation, from
            :func:`activation_elements_per_segment`. ``None`` leaves the batch to the population
            alone, which is the caller that has no model in hand.

    Returns:
        ``steps``, ``batch_size``, ``learning_rate``, ``eval_every`` and the ``epochs`` the step
        count came from, plus ``batch_size_source`` and the activation figure behind it -- so a run
        whose batch was lowered for memory says which of the two bounds decided it rather than
        leaving a reader to infer it from an unexpectedly large step count.
    """
    resolved_epochs = DEFAULT_FIT_EPOCHS if epochs is None else int(epochs)
    population_batch = max(
        1,
        min(
    ...
    resolved_batch, source = population_batch, "population"
    if activation_per_segment:
        affordable = max(1, MAX_FIT_ACTIVATION_ELEMENTS // int(activation_per_segment))
        if affordable < resolved_batch:
            resolved_batch, source = affordable, "activation ceiling"

    if steps is None:
        derived = math.ceil(resolved_epochs * max(1, int(n_fit_segments)) / resolved_batch)
        # The ceiling is on segment-visits rather than on iterations, so a batch lowered above buys
        # more steps rather than fewer passes over the fit half.
        ceiling = max(MIN_FIT_STEPS, MAX_FIT_SEGMENT_VISITS // resolved_batch)
        resolved_steps = int(min(ceiling, max(MIN_FIT_STEPS, derived)))
    else:
    ...
    return {
        ...
        "activation_elements": int(resolved_batch * int(activation_per_segment or 0)),
        "activation_ceiling": MAX_FIT_ACTIVATION_ELEMENTS,
    }
...
def run_oracle(
    ...
    *,
    ...
    budget = resolve_budget(
        ...
        # Sized for the **widest** probe that will be built, so one batch is safe for both fits and
        # the capacity comparison stays a comparison of widths rather than of step counts.
        activation_per_segment=activation_elements_per_segment(
            model,
            width_multiplier=CAPACITY_WIDTH_MULTIPLIER if capacity_check else 1,
        ),
    )
    # Only the four ``fit_probe`` takes. The rest of the budget -- the passes the step count was
    # derived from, and which of the two bounds set the batch -- is for the reader of the summary.
    resolved = {
        name: budget[name] for name in ("steps", "batch_size", "learning_rate", "eval_every")
    }
    shared = dict(model=model, likelihood=likelihood, device=device)
    ...
    return {
        ...
        # "The same decoder at the same capacity" as a **measurement** rather than a claim. The two
        # counts differ by exactly one layer's input width -- the production decoder projects from
        # $d_z$ and the probe from $d_{\mathrm{model}}$, which is the whole experiment -- so the
        # ratio is emitted and a reader can see how far "the same capacity" is from literal.
        "capacity_mirror": {
            "probe_parameters": parameter_count(probe),
            "decoder_parameters": parameter_count(model.decoder),
            "d_model": int(model.d_model),
            "d_z": int(model.d_z),
            "note": (
                "the probe is the production decoder class at the production hidden width, core "
                "depth, kernel, FiLM arrangement and horizon-attention depth; the two parameter "
                "counts differ only by the input projection, which reads d_model instead of d_z"
            ),
        },
        # The anchor geometry the probe was scored at, beside the model's own training tiling: the
        # two differ on every arm of this cell, and a score whose population was not stated could
        # not be read against the training CSV at all.
        "anchor_geometry": {
        ...
        "block_width": int(model.decoder.out_channels),

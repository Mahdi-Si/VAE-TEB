...
``False`` on a shape mismatch, so a test written the way the dense siblings' is would pass on a
...
with being correct -- and every shape criterion below holds vacuously on a control that permutes
nothing at all.
"""
...
def _model(perturb_posterior=None, **overrides) -> SeqVaeLagAttnCrws:
    torch.manual_seed(0)
...
def test_the_permuted_decode_has_the_same_shape_as_the_matched_one(perturb_posterior):
    ...
    mismatch -- so a version of this test written for a model that decodes every anchor would pass
    on a control that decoded 19 anchors against a 4-anchor forecast."""
    ...
    # The raw grid, not a resolved channel count: the last axis counts raw samples per horizon
    # token, which is the one width in this model no budget can move.
    assert permuted["mu_full"].shape[-1] == model.decoder_out_channels == model.raw_per_step
...
def test_the_anchor_keys_are_absent_from_the_recomputed_set_and_identical_between_the_dicts(
    ...
    the readout reports would be partly that -- and the anchored raw gather would then pull a
    different window for the two branches with every shape correct."""
    ...
    assert torch.equal(permuted["anchor_index"], out["anchor_index"])
...
def test_at_initialisation_every_criterion_above_would_hold_vacuously(perturb_posterior):
    """Why every test in this file perturbs first, stated as a passing test rather than as a
    comment. On a fresh model $q(z \\mid Y, U) = p(z \\mid Y)$ exactly, so a deranged source moves
    the posterior not at all: the KL is zero under both pairings, the shuffled block score equals
    the matched one, and a control that permuted nothing would satisfy every shape, identity and
    ordering assertion above."""
    fresh = _model()
    batch = make_stub_batch(_BATCH, TINY_SEQ_LEN)
    out = _forward(fresh, batch)
    permuted = _permute(fresh, out)

    matched = fresh.compute_loss(out, batch.fhr, weight=batch.weight)["metrics"]
    shuffled = fresh.compute_loss(permuted, batch.fhr, weight=batch.weight)["metrics"]

    assert float(matched["source_conditioned_kl_raw"]) == 0.0
    assert float(shuffled["source_conditioned_kl_raw"]) == 0.0

    # And the same model, perturbed, separates them -- so the vacuity above is the initialisation's
    # rather than this file's.
    perturb_posterior(fresh)
    moved_out = _forward(fresh, batch)
    moved = fresh.compute_loss(_permute(fresh, moved_out), batch.fhr, weight=batch.weight)
    assert float(moved["metrics"]["source_conditioned_kl_raw"]) > 0.0
...
def test_the_control_uses_a_real_derangement(perturb_posterior):
    """No fixed point: a sample paired with its own source is a matched forward wearing the
    control's name, and at $B = 4$ a fixed point is a real possibility rather than an arithmetic
    impossibility."""
    model = _model(perturb_posterior)
    out = _forward(model, make_stub_batch(_BATCH, TINY_SEQ_LEN))
...
def test_the_shuffled_readouts_separate_from_the_matched_ones_after_perturbation(
    task, perturb_posterior
):
    """The control's whole claim, and the one that is vacuous at initialisation: a stranger's source
    must give a different posterior and a worse forecast than the sample's own. Both halves, because
    a control that rebuilt the KL and not the decode would satisfy one of them."""
    module = task()
    perturb_posterior(module.orig_model)
    batch = make_stub_batch(_BATCH, TINY_SEQ_LEN)
...
def test_the_shuffled_penalty_is_the_gap_against_the_matched_full_score(task, perturb_posterior):
    """The reported penalty must be a difference of two scores of the *same* raw window at the
...
def test_the_control_leaves_the_prior_and_base_branch_bitwise_unchanged(task, perturb_posterior):
    ...
    a different anchor set on the two stages, so a train-versus-val comparison would differ for a
    reason that has nothing to do with the control. Every target-only readout must be **bitwise**

...
class PlSeqVaeClassifier(LightningModelBase):
    ...
    def compute_loss_and_metrics(self, batch, batch_idx, stage: str):
        ...
        sso = batch.second_stage_onset if hasattr(batch, 'second_stage_onset') else None

        # Forward pass through the model
        outputs = self.model(y_st=y_st, y_ph=y_ph, x_ph=x_ph, tlo=tlo, sso=sso)
...
class GraphModelClassifierTrainer(GraphModelBase):
    ...
    def create_model(self):
        ...
        if sso_embed_dim > 0:
            logger.info(f"SSO embedding enabled: embed_dim={sso_embed_dim}, "
                        f"classifier input_dim={classifier_input_dim}")

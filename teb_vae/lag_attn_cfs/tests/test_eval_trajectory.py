...
**And one thing is this cell's own: the profile starts at the anchor floor.** The causal front end
decodes no anchor below $F$, so there is no row to profile below it rather than a row scoring a
warm-up value -- which is a different figure from the raw cells' warm-up droop and a different
statement about what the reader is looking at. The floor is read off the run's own geometry record,
and the tests below cover the three states that produces: the two agreeing, a record that carries
no geometry at all (the offline re-run), and the two disagreeing.
"""
...
#: The shipped anchor floor of this cell. Written out rather than imported from the model, because
#: what the tests below check is that the analysis reports *the record's* number rather than a
#: constant of its own -- and importing the model's would make that indistinguishable.
ANCHOR_FLOOR = 133
...
def _context(per_anchor: pd.DataFrame, record: Optional[Dict[str, Any]] = None) -> Any:
    ...
    collection = types.SimpleNamespace(
        per_sample=pd.DataFrame(), per_anchor=per_anchor, record=record or {}, retained={},
        results={}, vectors={},
    )
...
# =============================================================================
# The anchor floor: this cell's own structural caveat
# =============================================================================
def test_the_floor_is_read_from_the_runs_own_geometry_record() -> None:
    """Not a constant of this module. An arm that moved the warm-up budget moves the floor, and a
    hard-coded 133 would then describe a different model from the one that was scored."""
    assert analysis.anchor_floor({"geometry": {"anchor_floor": ANCHOR_FLOOR}}) == ANCHOR_FLOOR
    assert analysis.anchor_floor({"geometry": {"anchor_floor": 30}}) == 30
    # The offline re-run's case: a record with no geometry block at all.
    assert analysis.anchor_floor({}) is None
    assert analysis.anchor_floor({"geometry": {}}) is None


def test_the_emitted_profile_starts_at_the_floor_and_says_so(tmp_path) -> None:
    r"""The within-segment caveat, made checkable. Nothing below $F$ is decoded, so the first
    anchor on the table *is* $F$ -- and the record carries both numbers rather than only the
    conclusion, so a reader can see which two things agreed."""
    per_anchor = _anchors(
        _segment("a", -4000.0, range(ANCHOR_FLOOR, ANCHOR_FLOOR + 12), value=1.0)
        + _segment("b", -9000.0, range(ANCHOR_FLOOR, ANCHOR_FLOOR + 12), value=2.0)
    )

    result = analysis.run_trajectory_analysis(
        _context(per_anchor, record={"geometry": {"anchor_floor": ANCHOR_FLOOR}}),
        eval_config={}, output_dir=tmp_path, probe=None,
    )
    profile = pd.read_csv(tmp_path / analysis.ANALYSIS_DIRNAME / analysis.WITHIN_SEGMENT_FILENAME)

    assert result["within_segment"]["anchor_floor"] == ANCHOR_FLOOR
    assert result["within_segment"]["first_anchor"] == ANCHOR_FLOOR
    assert result["within_segment"]["starts_at_floor"] is True
    # And on the emitted table itself, not only in the record derived from it.
    assert int(profile["anchor"].min()) == ANCHOR_FLOOR
    assert float(profile["seconds_in_segment"].min()) == pytest.approx(
        ANCHOR_FLOOR * SECONDS_PER_STEP
    )


def test_a_profile_that_does_not_start_at_the_floor_is_reported_rather_than_raised(
    tmp_path,
) -> None:
    """Non-vacuity for the case above, and the honest handling of a disagreement: the tables are
    what they are, and what a mismatch means is that they and the record describe different runs.
    Saying so beats raising after the expensive pass has already finished."""
    per_anchor = _anchors(_segment("a", -4000.0, range(ANCHOR_FLOOR + 4, ANCHOR_FLOOR + 12),
                                   value=1.0))
    ...
    assert result["within_segment"]["starts_at_floor"] is False
    assert result["within_segment"]["first_anchor"] == ANCHOR_FLOOR + 4


def test_a_record_without_geometry_reports_the_observed_anchor_and_no_verdict(tmp_path) -> None:
    """The offline re-run against a directory whose record predates the geometry block. The
    observed first anchor is still reported; what is absent is the *claim* that it is the floor."""
    per_anchor = _anchors(_segment("a", -4000.0, range(ANCHOR_FLOOR, ANCHOR_FLOOR + 6), value=1.0))

    result = analysis.run_trajectory_analysis(
        _context(per_anchor), eval_config={}, output_dir=tmp_path, probe=None
    )

    assert result["within_segment"]["anchor_floor"] is None
    assert result["within_segment"]["starts_at_floor"] is None
    assert result["within_segment"]["first_anchor"] == ANCHOR_FLOOR
...
# =============================================================================
@pytest.mark.slow
def test_the_real_run_assembles_every_recording(collected_run) -> None:
    """The fixture gives each recording several segments at different epochs, so the assembly has
    something to assemble and the summary's counts are checkable against the tables."""
    block = collected_run["summary"]["results"]["trajectory"]
    directory = collected_run["results_dir"] / analysis.ANALYSIS_DIRNAME
    ...
    assert block["composition"]["n_recordings"] == (
        collected_run["summary"]["results"]["n_recordings"]
    )
    assert block["whole_delivery"]["n_timesteps"] == len(trajectory)
    assert int(summary["n_segments"].sum()) == collected_run["summary"]["results"]["n_samples"]
...
@pytest.mark.slow
def test_the_real_runs_profile_starts_at_that_runs_own_floor(collected_run) -> None:
    r"""The structural caveat against a real pass. Asserted as the *identity* against the run's own
    geometry rather than against $133$: the fixture's model is at the tiny geometry, and what has
    to hold at every scale is that no anchor below the floor was decoded at all."""
    block = collected_run["summary"]["results"]["trajectory"]

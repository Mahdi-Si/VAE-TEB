...
def raw_to_decim_index(
    t_raw: np.ndarray, *, decim: int = 16, T_dec: int = 300,
) -> np.ndarray:
    """Map raw 4 Hz indices to the decimated grid.

    Both grids are aligned at sample 0; the model decimates 16:1 via
    integer division. Out-of-range indices clip to ``T_dec - 1``.

    Args:
        t_raw: Integer raw-sample indices.
        decim: Decimation factor (default 16).
        T_dec: Decimated sequence length (default 300).

    Returns:
        Integer decimated-grid indices of the same shape as ``t_raw``.
    """
    arr = np.asarray(t_raw, dtype=np.int64)
    out = np.minimum(arr // int(decim), int(T_dec) - 1)

r"""Journal-quality matplotlib style for ``synthetic_v2`` figures.

This module is the single source of truth for the look of every figure in the
``synthetic_v2`` raw$\to$scattering$\to$block-TE validation pipeline
(:mod:`visualize_v2` and :mod:`final_report_v2`). Call :func:`apply_style` once
before plotting to get a consistent, publication-grade look. It is a standalone
copy/adaptation of the older ``synthetic/plot_style.py`` (the module must not
import from the ``synthetic`` package at runtime, per the v2 standalone rule);
the ``M``-dilution semantic maps of the v1 mixed pipeline are dropped because v2
is single-pathway.
...
Project adaptations:
    ...
    * :func:`save_figure` defaults to ``("pdf", "png")`` -- a vector format for
      journal submission plus a high-resolution raster for slides/quick views.
...
# Prefer the rcParams sizes (omit ``fontsize``) and FONT_SUPTITLE for
# figure-level titles; these constants exist for the rare oversized-label case.
...
def apply_style() -> None:
    r"""Apply the publication rcParams to the global matplotlib state.
...
def style_axes(ax: Axes, *, grid: str = "major", minor_ticks: bool = True) -> None:
    r"""Apply consistent spine, grid, and tick styling to one axes.
...
def stacked_figure(
    ...
    *,
    ...
    r"""Build a two-column stacked-panel figure with aligned colorbar gutters.
...
def attach_colorbar(
    ...
    *,
    ...
    r"""Route a mappable's colorbar into a pre-allocated narrow axes.
...
def add_colorbar(
    ...
    *,
    ...
    r"""Append a slim colorbar to the right of a single axes.
...
def save_figure(
    ...
    *,
    ...
    r"""Save a figure to multiple formats and (optionally) close it.
    ...
    Note:
        This uses ``Path.with_suffix`` and therefore assumes ``path`` is a stem
        *without* a dotted extension. For stems that legitimately contain dots
        (e.g. ``G1_te2.0_D8``) use :func:`visualize_v2._resolve_output_paths`
        instead, which appends rather than replaces the suffix.
...
def tighten_xaxis(ax: Axes, x: np.ndarray) -> None:
    r"""Clamp the x-limits to the data range with zero padding.
...
def auto_ylim(
    ...
    *,
    ...
    r"""Set y-limits from data with a small symmetric padding.

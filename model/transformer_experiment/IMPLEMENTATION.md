...
| `anchor_indices` | `(B, K)` e.g. `(B, 4)` | Sampled anchor positions. Values in `[29, 265]` using 0-based indexing. LongTensor. |
...
In inference mode, the TE posterior means are computed on a fixed 0-based anchor grid (every 15 steps from index 29 to 265, giving 16 anchors: `[29, 44, 59, ..., 254]`).
...
|------|-------|-------------|
| `training=True` | `(B, K)` e.g. `(B, 4)` | Stochastically sampled 0-based anchors. Values in `[29, 265]`. |
| `training=False` | `(B, K_grid)` e.g. `(B, 16)` | Fixed 0-based grid: `[29, 44, 59, ..., 254]`. Same for all batch elements. |

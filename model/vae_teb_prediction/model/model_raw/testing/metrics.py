...
def compute_kld(
    ...
    Uses the closed-form diagonal-Gaussian KL divergence

    $$
    \mathrm{KL}(q \,\|\, p) = \tfrac12 \sum_d \left[
        \log\frac{\sigma^2_{p,d}}{\sigma^2_{q,d}}
        + \frac{\sigma^2_{q,d}}{\sigma^2_{p,d}}
        + \frac{(\mu_{q,d} - \mu_{p,d})^2}{\sigma^2_{p,d}} - 1 \right],
    $$

    where $p$ is the prior and $q$ the posterior. The KL measures transfer entropy: how much
    information the source signal (UP) provides beyond what the target (FHR) already contains.
    ...
    # Closed-form diagonal-Gaussian KL(q || p); see the docstring for the formula.
    kld = (
...
def compute_raw_forecast_metrics(
    mu_full: Tensor,
    x_plus: Tensor,
    warmup: int,
    horizon: int,
    *,
    ...
    r"""Per-sample raw-waveform forecast quality metrics (S6-T03).

    The raw analogue of :func:`compute_forecast_metrics`. It evaluates the raw forecast
    ``mu_full`` against the crop-aligned future raw-FHR block ``x_plus`` over the valid anchor range
    $[w, T-H)$. There is **no** scattering / phase channel split -- the last axis $R$ is a block of
    raw substeps of a single waveform, not a channel dimension, so a single-block reduction is the
    correct summary. Alongside VAF / MSE / SNR (via :func:`compute_reconstruction_metrics`) and a
    per-sample $R^2$, it reports a **multi-scale low-pass MSE**: the forecast is block-averaged at
    each of ``lowpass_scales`` seconds (block width $= \text{scale} \cdot f_s$ raw samples along the
    contiguous $H\!\cdot\!R$ future axis) and the squared error of the block means is averaged --
    scoring the deceleration *trend* the model can forecast rather than the per-sample jitter it
    cannot (roadmap §10).

    Args:
        mu_full: Raw forecast mean $(B, T, H, R)$ -- the model emits this on the **full** $T$
            anchor axis (``forward`` does not slice to $T_{\mathrm{valid}}$).
        x_plus: Ground-truth future raw-FHR block $(B, T_{\mathrm{valid}}, H, R)$ from
            :meth:`TestRunner.build_future_target` ($T_{\mathrm{valid}} = T - H$).
        warmup: Warm-up anchors to skip.
        horizon: Forecast horizon $H$ (must equal ``mu_full.size(2)``).
        lowpass_scales: Block-average scales in seconds.
        fs: Raw sampling rate in Hz (raw FHR is $4$ Hz).

    Returns:
        Dict of per-sample tensors (all shape $(B,)$ unless noted): ``raw_vaf``, ``raw_mse``,
        ``raw_snr``, ``raw_r2``, ``raw_mse_per_horizon`` $(B, H)$, and ``raw_lowpass_mse``.
    """
    B, T, H, R = mu_full.shape
    if int(H) != int(horizon):
        raise ValueError(
            f"horizon mismatch: mu_full.size(2)={H}, horizon={horizon}"
        )

    start, T_valid = _feature_valid_slice(warmup, horizon, T)
    # x_plus already carries exactly T - H anchors; slice both to [start:T_valid] so anchor t of the
    # prediction lines up with anchor t of the target.
    mu_valid = mu_full[:, start:T_valid, :, :]
    x_valid = x_plus[:, start:T_valid, :, :]

    if mu_valid.numel() == 0 or x_valid.numel() == 0:
        zeros = torch.zeros(B, device=mu_full.device, dtype=mu_full.dtype)
        return {
            "raw_vaf": zeros,
            "raw_mse": zeros.clone(),
            "raw_snr": zeros.clone(),
            "raw_r2": zeros.clone(),
            "raw_mse_per_horizon": torch.zeros(
                B, int(H), device=mu_full.device, dtype=mu_full.dtype
            ),
            "raw_lowpass_mse": zeros.clone(),
        }

    # VAF / MSE / SNR over the full valid block (reduces every non-batch axis).
    recon = compute_reconstruction_metrics(x_valid, mu_valid)

    diff = mu_valid - x_valid
    sq = diff.pow(2)
    raw_mse_per_horizon = sq.mean(dim=(1, 3))          # (B, H)

    # Per-sample R^2 against the mean of the valid target.
    y_flat = x_valid.reshape(B, -1)
    mu_flat = mu_valid.reshape(B, -1)
    y_mean = y_flat.mean(dim=1, keepdim=True)
    ss_res = (mu_flat - y_flat).pow(2).sum(dim=1)
    ss_tot = (y_flat - y_mean).pow(2).sum(dim=1).clamp_min(1e-12)
    raw_r2 = (1.0 - ss_res / ss_tot).clamp(min=-10.0, max=1.0)

    # Multi-scale low-pass MSE over the contiguous H*R future axis.
    fut_mu = mu_valid.reshape(B, mu_valid.size(1), H * R)
    fut_x = x_valid.reshape(B, x_valid.size(1), H * R)
    length = H * R
    per_scale = []
    for scale in lowpass_scales:
        width = max(1, min(int(round(float(scale) * fs)), length))
        n_blocks = length // width
        if n_blocks == 0:
            continue
        mu_b = fut_mu[..., : n_blocks * width].reshape(B, -1, n_blocks, width).mean(dim=-1)
        x_b = fut_x[..., : n_blocks * width].reshape(B, -1, n_blocks, width).mean(dim=-1)
        per_scale.append((mu_b - x_b).pow(2).mean(dim=(1, 2)))  # (B,)
    if per_scale:
        raw_lowpass_mse = torch.stack(per_scale, dim=0).mean(dim=0)
    else:
        raw_lowpass_mse = torch.zeros(B, device=mu_full.device, dtype=mu_full.dtype)
    ...
    return {
        ...
        "raw_snr": recon["snr"],
        "raw_r2": raw_r2,
        "raw_mse_per_horizon": raw_mse_per_horizon,
        "raw_lowpass_mse": raw_lowpass_mse,
    }
...
def _split_channel_blocks(per_elem: Tensor) -> Dict[str, Tensor]:
    r"""Reduce an elementwise score $(B, T_v, H, R)$ to ``total`` / ``per_horizon`` summaries.

    Raw port (S6-T05): the last axis $R$ is a block of raw substeps of a **single** waveform, not a
    scattering + phase channel concatenation, so there is no meaningful ``st`` / ``ph`` split -- the
    whole future block is one block. Only ``total`` (mean over anchors, horizon, substeps) and
    ``per_horizon`` (mean over anchors, substeps) are returned.
    """
    return {
...
def _empty_score(B: int, horizon: int, prefix: str, ref: Tensor) -> Dict[str, Tensor]:
    """Zero-filled result for a degenerate (empty) anchor range (raw: total / per_horizon only)."""

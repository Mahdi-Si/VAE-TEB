...
The conv-LSTM causal cell has to qualify both claims with ``causal_norm: true``: without it a
time-pooling normaliser inside each encoder mixes the whole sequence, the "prior" conditions on the
future, and the source-conditioned KL is not a coupling readout at all. This architecture has no
such normaliser -- RMSNorm reduces over channels only, the convolutions pad left, and the attention

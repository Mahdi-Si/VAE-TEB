r"""The tiled anchor set: which anchors are decoded, and what the padded slots are for.

Windows of consecutive anchors overlap $(H-1)/H$, so a model decoding every anchor scores each
target coefficient up to $H$ times per step and correlates the gradients inside one batch element.
This family decodes a *tile* instead -- a phase and a stride -- so the windows partition the
timeline within a step, and every anchor is still reached over epochs because the phase moves.

Three properties carry that, and each replaces something that does not work:

**$A_{\max}$ is a geometry constant.** $\lceil (T_{\mathrm{valid}} - F)/S \rceil$, independent of the
phase and of the batch, so no rank can disagree about a shape and no shape is a function of the
data. What varies is how many entries are real.

**Padding repeats the last valid anchor.** A padded slot holding a distinct *legal* index would
produce a fully live forecast-mask row, so its target block would be gathered and scored twice --
while the KL support, which is a set, would count it once. The reconstruction and KL denominators
would diverge and $\beta$ would quietly stop meaning what it means in every other cell of the grid.

**Nothing is drawn.** The phase arrives already derived. A ``torch.randint`` inside the forward
would consume the global RNG stream, move the reparameterisation $\epsilon$, break every bitwise
comparison in the suite and fail to survive a checkpoint resume -- with $A_{\max}$ a constant either
way, so no shape would say so.

Every number below is derived from the geometry rather than written out, so a horizon or floor
change re-derives them instead of failing an unrelated literal.
"""
...
def test_the_anchor_axis_is_a_geometry_constant(model) -> None:
    ...
    wider, _ = model._build_anchor_index(
        batch=7, device=torch.device("cpu"), anchor_phase=0
    )
...
def test_valid_entries_are_ascending_and_inside_the_anchor_range(model) -> None:
    """Every index in range, ascending among the real entries, and none below the floor."""
...
def test_short_rows_repeat_their_last_valid_anchor(model) -> None:
    ...
    floor, t_valid, stride = _geometry(model)
    # The phase whose count is smallest is the largest one, which is where padding appears at all.
    index, valid = model._build_anchor_index(
...
def test_every_phase_is_a_different_grid_and_together_they_cover_everything(model) -> None:
    """Goal of the tiling: no coefficient scored twice in a step, every anchor reached over epochs."""
    floor, t_valid, stride = _geometry(model)
    covered: set = set()
    grids = []
    for phase in range(stride):
        ...
        row = set(index[0][valid[0]].tolist())
        grids.append(row)
        covered |= row

    assert covered == set(range(floor, t_valid))
...
def test_a_missing_phase_is_refused_once_the_stride_is_real(model) -> None:
    """A forgotten phase would train every sample of every epoch on one grid, at a fixed offset
    from the segment start, with no shape and no count differing."""
...
def test_a_non_zero_phase_at_stride_one_is_refused(model) -> None:
    r"""$\mathcal A(\varphi)$ truncates rather than rotating, so a phase at stride $1$ would
    silently drop its first $\varphi$ anchors and shorten the anchor count."""
...
def test_a_phase_at_or_above_the_stride_is_refused(model) -> None:
    ...
    with pytest.raises(ValueError, match=str(stride)):
        model._build_anchor_index(
            batch=BATCH, device=torch.device("cpu"), anchor_phase=stride
        )
    with pytest.raises(ValueError, match="-1"):
        model._build_anchor_index(
            batch=BATCH, device=torch.device("cpu"), anchor_phase=-1
        )
...
def test_a_forward_consumes_the_same_randomness_at_every_phase(model) -> None:
    """The end-to-end form: one draw, the reparameterisation's, whatever the tiling is."""
    _, _, stride = _geometry(model)
    streams = make_streams(tiny_warmup_kwargs())
    states = []
    for phase in range(stride):
        torch.manual_seed(0)
        with torch.no_grad():
...
def test_the_decoder_is_invoked_on_the_gathered_latents(model) -> None:
    ...
    for latent, key in zip(seen, ("z_prior", "z_post")):
        expected = out[key].gather(
            1, index[:, :, None].expand(-1, -1, model.d_z)
        )

...
geometry that is $152$ positions read at anchors $133 \dots 284$, so the first read is out of range,
...
def test_it_renders_at_the_shipped_geometry(task):
    ...
    floor of $133$ and $152$ decoded anchors tiled into $11$ windows of $240$ raw samples. A page
    ...
    try:
        ...
        assert int(valid.sum()) == geometry.t_valid - geometry.warmup == 152
        assert [int(anchors[position]) for position in positions] == list(
            range(geometry.warmup, geometry.t_valid, geometry.horizon)
        )
        ax = _axes_titled(figure, "Forecast")
        assert f"{len(positions)} of 152 decoded anchors drawn" in ax.get_title()
        assert _labelled(ax, "decoded anchors")[0].get_xdata().size == 152

...
from teb_vae.lag_attn_rws.eval.metrics import NORMALISED_UNIT

#: A loader statistics block with a **non-zero mean**, which is what separates the scale-only
#: inversion an RMSE needs from the affine one a level needs. With a zero mean the two agree and
#: the test would pass under the wrong conversion.
STATS = {"fhr": {"mean": 140.0, "std": 10.0}}
...
# =============================================================================
def test_a_rooted_metric_is_rooted_and_converted_as_a_spread(per_sample) -> None:
    r"""Scale only, no offset. An RMSE is a difference of levels, so the normalisation's additive
    term cancels; put through the *level* conversion an RMSE of $0.1$ z-units becomes $\approx 141$
    bpm instead of $\approx 1$, which is the plausible-looking wrong answer."""
    segment, _, units = distributions.build_frames(per_sample, STATS)

    squares = np.asarray(per_sample["sq_error_full"], dtype=np.float64)
    expected = np.sqrt(squares) * (10.0 + 1e-8)

    assert units["rmse_full"] == "bpm"
    assert np.allclose(np.asarray(segment["rmse_full"]), expected)
    # The affine conversion would have added the 140 bpm mean to every value.
    assert float(np.max(segment["rmse_full"])) < 100.0
...
def test_the_two_levels_root_at_different_points(per_sample) -> None:
    ...
    segment, recording, _ = distributions.build_frames(per_sample, STATS)

    guid = "g_h1"
    squares = per_sample.loc[per_sample["guid"] == guid, "sq_error_full"]
    hand_rooted_once = np.sqrt(float(squares.mean())) * (10.0 + 1e-8)
    mean_of_roots = float(
...
def test_the_summary_carries_both_levels_for_every_cell(per_sample) -> None:
    segment, recording, units = distributions.build_frames(per_sample, STATS)
...
class _Collection:
    ...
    #: Sentinel, so a caller can pass an *empty* record -- a run whose statistics are unknown --
    #: and get one, rather than the default a falsy check would substitute back in.
    _DEFAULT = object()

    def __init__(self, per_sample, record=_DEFAULT):
        self.per_sample = per_sample
        self.record = {"normalization": STATS} if record is self._DEFAULT else record
...
def _run(frame, tmp_path, record=_Collection._DEFAULT):
    """Run the analysis. The default carries :data:`STATS`, so a test that passes ``record={}``
    is genuinely exercising the no-statistics path rather than repeating the default."""
...
def test_the_derived_csv_is_the_figures_reproducible_from_disk(per_sample, tmp_path) -> None:
    """The drawn units are not on ``per_sample.csv`` -- the bpm columns are derived here -- so an
    offline reader needs them written down rather than recomputable only through this module."""
...
def test_a_run_without_normalisation_statistics_still_completes(per_sample, tmp_path) -> None:
    result = _run(per_sample, tmp_path, record={})

    units = {entry["metric"]: entry["unit"] for entry in result["metrics"]}
    assert units["rmse_full"] == NORMALISED_UNIT

r"""The evaluation command line for this cell: a checkpoint in, ``summary.json`` out.

.. code-block:: bash

    python -m teb_vae.lag_attn_transformer_cfs.eval.run --checkpoint <path> [--output-dir <dir>]

**Every line of the pipeline behind this is the cfs cell's.**
:func:`teb_vae.lag_attn_cfs.eval.run.main` takes the model it is evaluating as a
:class:`~teb_vae.lag_attn_cfs.eval.binding.ModelBinding`, so what this module supplies is
:data:`~teb_vae.lag_attn_transformer_cfs.eval.binding.TRF_CFS_BINDING` and a ``prog=`` string. It is
not a thin wrapper for tidiness: the two cells exist to be compared -- that is what the cross-cell
table in ``verify.py`` is for -- and a second copy of the runner is how two things that must stay
comparable stop being comparable, because the first fix to an analysis lands on one side and the two
summaries quietly stop meaning the same thing.

What the cfs module's docstring says about a run therefore holds here verbatim: the resolved config
beside the checkpoint is the authority and this package's committed delta is merged over it, the
loader is forced single-process and fixed-seed shuffled, the forward is called densely at
``anchor_phase=0, anchor_stride=1`` regardless of the tiling the run trained at, the pass happens
once per run directory and writes durable tables, every analysis runs inside a failure-isolating
wrapper while preflight deliberately does not, and a checkpoint is not required for a re-run against
...
enumerated here rather than borrowed: a borrowed parser would list the cfs cell's registry under
this cell's usage line, and the two are only identical because a test says so rather than because
one is the other.
"""
...
#: Repository root: ``teb_vae/lag_attn_transformer_cfs/eval/run.py`` -> up four.
...
from teb_vae.lag_attn_transformer_cfs.eval.binding import TRF_CFS_BINDING  # noqa: E402

#: Re-exported so a caller of this entry point does not have to reach into the cfs cell for the
...
#: Steps that always run and are **not** selectable. The cfs cell's, unchanged: the target channel
#: map describes the *data* -- the shards' own ``sel_*`` provenance and per-block causal attributes
#: -- so a model-specific addition here would be a category error.
...
def analysis_registry() -> Dict[str, Any]:
    ...
    """
    return shared_run.merged_analysis_functions(TRF_CFS_BINDING)
...
def main(*args: Any, **kwargs: Any) -> int:
    """Evaluate a conv-Transformer cfs checkpoint, or re-read a finished run of one.
    ...
    Returns:
        ...
        flag -- that asymmetry is why ``verify.py`` exists separately.
    """
    kwargs.setdefault("binding", TRF_CFS_BINDING)
    return shared_run.main(*args, **kwargs)


def build_parser() -> argparse.ArgumentParser:
    """Build the command-line parser: the cfs cell's flags, this package's name and registry.

    The flags are enumerated here rather than borrowed from the cfs parser because ``--only`` and
    ``--skip`` interpolate *this* model's registry into their help text, and a borrowed parser
    would name the other cell's.

    Returns:
        The parser. Every ``dest`` is also a valid :data:`RUN_ARGS` key.
    """
    parser = argparse.ArgumentParser(
        prog="python -m teb_vae.lag_attn_transformer_cfs.eval.run",
        description=f"Evaluate a trained {TRF_CFS_BINDING.model_cls.__name__} checkpoint.",
    )
    parser.add_argument(
...
def _cli(argv: Optional[List[str]] = None) -> int:
    ...
    # The cfs cell's resolver, handed this package's parser: the merge rule, the source record and
...
#: margin belong in the override delta (``configs/eval_overrides.yaml``), which is dumped into the
#: run directory as the durable record; a value injected from Python would appear in no artifact
#: and could not be recovered from the output afterwards.
RUN_ARGS: Dict[str, Any] = {
    ...
    # over the shards.
    #
    # The list is the cfs cell's, entry for entry: this cell registers no analysis of its own,
    # because replacing both history encoders changes what produces the numbers and not which
    # numbers there are. ``teb_vae/lag_attn_cfs/eval/run.py`` documents what each one answers, and
    # a second gloss here would be a second thing to keep true.
    "only": None,
    "skip": None,
}

...
is the comparison model's task plus a learning-rate schedule. That makes drift here a question
about *this* model's metrics dict rather than about a list, which is why the coverage assertions
drive the real task rather than restating what it ought to emit.
...
def test_the_gradient_norm_column_is_tracked_train_only():
    """It is what the provisional ``gradient_clip_val`` gets re-derived from, and it exists on the
    training path alone -- a ``val/`` variant would be NaN in every row of every run."""
...
def test_the_checkpoint_filename_carries_this_models_stem_and_no_double_prefix(built):
    ...
    assert filename == "lag-attn-trf-rws-{epoch:02d}"
...
def test_the_plotting_block_name_is_the_inherited_drivers_literal():
    ...
    would show."""
    assert LagAttnTrfRwsTrainer.PLOT_CONFIG_KEY == PLOTTING_BLOCK == LagAttnRwsTrainer.PLOT_CONFIG_KEY

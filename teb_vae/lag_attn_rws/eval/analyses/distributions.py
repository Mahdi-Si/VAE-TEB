...
* **The error metrics are rooted per segment and converted to bpm**, because a distribution of
  ...
  Jensen-biased low, and the object drawn here is the distribution itself rather than its mean. The
  recording-level series still roots **after** the per-recording mean, exactly as the rest of the
  pipeline does -- so the two levels differ by that bias as well as by the aggregation, and the
  emitted record says so rather than leaving the two to be compared as though they were the same
  arithmetic.
"""
...
class Metric:
    ...
    Attributes:
        ...
        root: Whether the source is an unrooted mean square that must be rooted and then converted
            as a **spread** -- scale only, no offset, which is what
            :func:`~teb_vae.lag_attn_rws.eval.metrics.sigma_to_bpm` exists to get right.
        unit: The unit when no conversion applies. A rooted metric upgrades to ``'bpm'`` whenever
            the loader's statistics are known and keeps this otherwise.
...
METRICS: Tuple[Metric, ...] = (
    ...
    Metric(
        ...
        "per-segment RMSE of the source-conditioned forecast over its scored raw samples",
    ),
    Metric(
...
# =============================================================================
def build_frames(
    per_sample: pd.DataFrame, normalization: Optional[Dict[str, Any]]
    ...
    Args:
        ...
        normalization: The loader's FHR statistics, or ``None``. Only the rooted metrics use it.
    ...
    Returns:
        ...
        each metric to the unit it actually ended up in, which for a rooted metric depends on
        whether the statistics were available.
    """
    ...
    for metric in METRICS:
        ...
        if metric.root:
            ...
            # The *sigma* inversion: an RMSE is a difference of levels, so the normalisation's
            # additive term cancels and only its scale survives. The affine conversion is the
            # plausible-looking wrong answer and would land an RMSE of 0.1 z-units near 140 bpm.
            segment_values, unit = sigma_to_bpm(segment_values, normalization)
            recording_values, _ = sigma_to_bpm(recording_values, normalization)
...
def run_distributions_analysis(
    ...
    *,
    ...
    Args:
        context: The analysis context, read for the per-sample table and the loader statistics the
            bpm columns need.
    ...
    segment, recording, units = build_frames(
        per_sample, dict(record.get("normalization") or {})
    )

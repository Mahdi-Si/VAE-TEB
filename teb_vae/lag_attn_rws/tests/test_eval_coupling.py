...
negative; and a missing $H \cdot R$ being replaced by a guessed block size, which would rescale
the likelihood-space answer with nothing raising.
"""
...
# The percentages
# =============================================================================
#: A collection record the likelihood-space percentage is defined under: the shipped block size
#: $H \cdot R = 30 \times 16 = 480$, and a likelihood whose block score is a log-density.
GEOMETRY = {
    "geometry": {"horizon": 30, "raw_per_step": 16},
...
def test_the_error_space_percentages_are_the_documented_ratios() -> None:
    ...
    columns = coupling_analysis.percent_columns(per_guid, samples_per_anchor=480.0)
...
def test_the_likelihood_percentage_is_the_per_sample_density_ratio() -> None:
    r"""$100(e^{\Delta/HR} - 1)$, and exactly $0$ at $\Delta = 0$ -- the natural zero that makes
    this a percentage at all."""
    per_guid = _per_guid(mc_pred_gap=[0.0, 4.8])

    columns = coupling_analysis.percent_columns(per_guid, samples_per_anchor=480.0)

    # The second value hand-computed rather than restated as the implementation's own expression,
    # which would assert only that the line equals itself.
...
def test_an_mse_checkpoint_gets_no_likelihood_percentage() -> None:
    ...
    ``exp(gap / HR)`` is not a density ratio, and reporting it as "extra probability density"
    would be a false statement about the run rather than an imprecise one."""
    support = coupling_analysis.likelihood_percent_support(
        {"geometry": {"horizon": 30, "raw_per_step": 16}, "likelihood": "mse"}
    )

    assert support["skipped"] is True
    assert support["raw_samples_per_anchor"] is None
    ...
    columns = coupling_analysis.percent_columns(
        _per_guid(sq_error_base=[4.0], sq_error_full=[1.0], mc_pred_gap=[4.8]),
        samples_per_anchor=support["raw_samples_per_anchor"],
    )
...
def test_a_degenerate_block_size_is_refused_like_an_absent_one() -> None:
    """A geometry present but naming a zero-length forecast block would divide by zero."""
    assert coupling_analysis.raw_samples_per_anchor(
        {"geometry": {"horizon": 0, "raw_per_step": 16}}
    ) is None
...
def test_the_run_records_why_the_likelihood_percentage_was_skipped(tmp_path) -> None:
    ...
    result = coupling_analysis.run_coupling_analysis(
        _context(
            per_sample,
            record={"geometry": {"horizon": 30, "raw_per_step": 16}, "likelihood": "mse"},
        ),
...
def test_absent_geometry_skips_the_likelihood_percentage_rather_than_guessing_a_block_size() -> None:
    """$H \\cdot R$ is the denominator that turns a block score into a per-sample one. A guessed
    constant there would rescale the answer with nothing raising, so an unavailable geometry
    produces no column, no row and no headline key."""
    per_guid = _per_guid(mc_pred_gap=[1.0, 2.0], sq_error_base=[4.0, 4.0], sq_error_full=[1.0, 1.0])

    assert coupling_analysis.raw_samples_per_anchor({}) is None
    assert coupling_analysis.raw_samples_per_anchor(GEOMETRY) == 480.0
    assert coupling_analysis.likelihood_percent_support({})["raw_samples_per_anchor"] is None
...
def test_the_percent_figure_separates_the_two_spaces() -> None:
    """A per-sample density ratio and an error reduction differ by an order of magnitude, so one
    shared axis would flatten whichever is smaller into a line at zero."""
    from teb_vae.lag_attn.eval import figures as shared_figures
...
def test_the_analysis_writes_the_percentages_it_can_compute(tmp_path) -> None:
    ...
    assert percent["likelihood_space"]["raw_samples_per_anchor"] == 480.0

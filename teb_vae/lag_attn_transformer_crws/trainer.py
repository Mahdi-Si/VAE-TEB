...
against.** The warm-up budget resolution, the six pre-flight refusals, the callback assembly, the
...
:class:`~teb_vae.lag_attn_crws.trainer.LagAttnCrwsTrainer` and
...
* ``TARGET_FIELDS``, ``TRACKED_METRICS`` and ``preflight`` come from the **causal-input** parent,
  which is what the target domain and the input domain together decide. ``TARGET_FIELDS`` is
  ``("fhr",)`` on both parents and the conv-Transformer one would serve equally, but
  ``TRACKED_METRICS`` and ``preflight`` are not: without the causal parent's ``preflight`` a
  two-sided shard would be scored happily, and without its three added suffixes the run's
  ``metrics_history.csv`` would lack the geometry guard that says the tiling fired.
...
#: Repository root: ``teb_vae/lag_attn_transformer_crws/trainer.py`` -> up three.
...
# `python -m teb_vae.lag_attn_transformer_crws.trainer` from the repo root sets __package__ and needs
...
from teb_vae.lag_attn_crws.trainer import LagAttnCrwsTrainer  # noqa: E402
from teb_vae.lag_attn_rws.trainer import main as run_training  # noqa: E402
from teb_vae.lag_attn_transformer_crws.nets.model import SeqVaeLagAttnTrfCrws  # noqa: E402
from teb_vae.lag_attn_transformer_crws.task import SeqVaeLagAttnTrfCrwsTask  # noqa: E402
...
class LagAttnTrfCrwsTrainer(LagAttnCrwsTrainer, LagAttnTrfRwsTrainer):
    ...
    * omit ``CHECKPOINT_STEM`` and it writes ``lag-attn-crws-*.ckpt``, interleaving two models'
...
def main(config_path: str) -> None:
    ...
    """
    run_training(config_path, trainer_cls=LagAttnTrfCrwsTrainer)
...
RUN_CONFIG: str | None = "teb_vae/lag_attn_transformer_crws/configs/default.yaml"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "teb_vae/lag_attn_transformer_crws/configs/default.yaml. Run from the repo root. Optional "

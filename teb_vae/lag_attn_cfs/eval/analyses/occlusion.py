...
from teb_vae.lag_attn_cfs.eval._reuse import figures
...
def build_summary(per_horizon: pd.DataFrame, bands: Dict[str, Tuple[int, int]]) -> pd.DataFrame:
    """One row per band: its total delta, where over the horizon it peaks, and its live fraction.
...
def run_occlusion_analysis(
    ...
    *,
    ...
    summary = build_summary(per_horizon, bands)
    ...
    return {
        ...
        "files": [
            PER_RECORDING_FILENAME, PER_HORIZON_FILENAME, SUMMARY_FILENAME, figure_name
        ],
    }

...
class TargetEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.mu_layer = ResidualMLP(
            input_dim=32,
            hidden_dims=geometric_schedule(32, latent_dim, 5),
            final_activation=False,
            activation=nn.GELU
        )
...
# class InvStModule(nn.Module):
#     """
#     Inverse Scattering Transform Module to reconstruct raw signal from scattering features.
#     Placeholder implementation - replace with actual inverse scattering logic.
#     """
#     def __init__(self, input_dim: int = 40, latent_dim: int = 16):
#         super(InvStModule, self).__init__()
#         self.input_dim = input_dim
#         self.upsample_1 = nn.ConvTranspose1d(input_dim, 32, kernel_size=4, stride=2, padding=1)
#         self.norm_1 = nn.GroupNorm(num_groups=8, num_channels=32)
#         self.temporal_refine_1 = nn.Conv1d(32, 32, kernel_size=7, dilation=2, padding=6)

#         self.upsample_2 = nn.ConvTranspose1d(32, 32, kernel_size=4, stride=2, padding=1)
...
#         self.skip_to_x1 = nn.Linear(latent_dim, 32) 
#         self.skip_to_x2 = nn.Linear(latent_dim, 32)
#         self.skip_to_x3 = nn.Linear(latent_dim, 16)
...
#     def forward(self, st_coeffs: torch.Tensor, latent_z: torch.Tensor) -> torch.Tensor:
#         B, T, L = st_coeffs.shape
#         z_flat = latent_z.view(B*T, -1)
#         st_ph_coefs = st_coeffs.view(B, T, self.input_dim, -1)  # (B, T, 40, prediction_window)
#         st_ph_coefs = st_ph_coefs.view(B*T, self.input_dim, -1)  # (B*T, 40, prediction_window)
#         # st_ph_coefs = st_ph_coefs.transpose(1, 2)   # (B*T, prediction_window, 40)
#         x1 = F.gelu(self.norm_1(self.upsample_1(st_ph_coefs)))
#         x1 = F.gelu(self.temporal_refine_1(x1))
#         skip1 = self.skip_to_x1(z_flat).unsqueeze(-1)
#         skip1 = F.interpolate(skip1, size=x1.size(2), mode='linear', align_corners=False)
#         x1 = x1 + skip1

#         x2 = F.gelu(self.norm_2(self.upsample_2(x1)))
#         x2 = F.gelu(self.temporal_refine_2(x2))
#         skip2 = self.skip_to_x2(z_flat)
#         skip2 = F.interpolate(skip2, size=x2.size(2), mode='linear', align_corners=False)
#         x2 = x2 + skip2

#         x3 = F.gelu(self.norm_3(self.upsample_3(x2)))
#         x3 = F.gelu(self.temporal_refine_3(x3))
#         skip3 = self.skip_to_x3(z_flat)
#         skip3 = F.interpolate(skip3, size=x3.size(2), mode='linear', align_corners=False)
#         x3 = x3 + skip3

#         x4 = F.gelu(self.norm_4(self.upsample_4(x3)))
#         x4 = F.gelu(self.temporal_refine_4(x4))

#         refined = self.refine_stack(x4)
#         features = self.final_conv(refined)

#         mu = self.signal_mu(features).squeeze(1)
#         logvar = self.signal_logvar(features).squeeze(1)
...
class InvStModule(nn.Module):
    """
    Inverse Scattering Transform Module to reconstruct raw signal from scattering features.
    Uses 1D transposed convolutions over time and bicubic interpolation for skip connections.
    """

    def __init__(self, input_dim: int = 40, latent_dim: int = 16):
        super().__init__()
        self.input_dim = input_dim  # number of channels in scattering features (40)

        # Upsampling path (1D over time)
        self.upsample_1 = nn.ConvTranspose1d(input_dim, 32, kernel_size=4, stride=2, padding=1)
        self.norm_1 = nn.GroupNorm(num_groups=8, num_channels=32)
        self.temporal_refine_1 = nn.Conv1d(32, 32, kernel_size=7, dilation=2, padding=6)
        ...
        # Skip projections from latent_z
        self.skip_to_x1 = nn.Linear(latent_dim, 32)
        self.skip_to_x2 = nn.Linear(latent_dim, 32)
        self.skip_to_x3 = nn.Linear(latent_dim, 16)
    ...
    @staticmethod
    def _bicubic_1d(skip: torch.Tensor, target_len: int) -> torch.Tensor:
        """
        Effectively 1D bicubic interpolation by treating (L) as H in a (H, W=1) image.
        ...
        """
        n, c, l = skip.shape
        # Make it 4D: (N, C, H=L, W=1)
        skip_4d = skip.unsqueeze(-1)
        # Bicubic expects 4D and interpolates over (H, W)
        up_4d = F.interpolate(
            skip_4d,
            size=(target_len, 1),
            mode="bicubic",
        ...
        # Back to 1D over time: (N, C, target_len)
        return up_4d.squeeze(-1)

    def forward(self, st_coeffs: torch.Tensor, latent_z: torch.Tensor):
        """
        Args:
            st_coeffs: (B, T, 1200)  where 1200 = input_dim * prediction_window (40 * 30)
            latent_z:  (B, T, latent_dim)

        Returns:
            mu, logvar: (B, T, 480)
        """
        B, T, L = st_coeffs.shape
        if L % self.input_dim != 0:
            raise ValueError(f"Last dim {L} must be divisible by input_dim={self.input_dim}")
        pred_window = L // self.input_dim  # e.g. 30

        # ---- Prepare input for 1D convs: (B*T, C=40, L=pred_window=30) ----
        x = st_coeffs.view(B, T, self.input_dim, pred_window)   # (B, T, 40, 30)
        x = x.view(B * T, self.input_dim, pred_window)          # (B*T, 40, 30)

        # Flatten latent for skips: (B*T, latent_dim)
        z_flat = latent_z.view(B * T, -1)

        # ---- Block 1: 30 -> 60 ----
        x1 = F.gelu(self.norm_1(self.upsample_1(x)))            # (B*T, 32, 60)
        x1 = F.gelu(self.temporal_refine_1(x1))                 # (B*T, 32, 60)

        skip1 = self.skip_to_x1(z_flat).unsqueeze(-1)           # (B*T, 32, 1)
        skip1 = self._bicubic_1d(skip1, x1.size(2))             # (B*T, 32, 60)
        x1 = x1 + skip1

        # ---- Block 2: 60 -> 120 ----
        x2 = F.gelu(self.norm_2(self.upsample_2(x1)))           # (B*T, 32, 120)
        x2 = F.gelu(self.temporal_refine_2(x2))                 # (B*T, 32, 120)

        skip2 = self.skip_to_x2(z_flat).unsqueeze(-1)           # (B*T, 32, 1)
        skip2 = self._bicubic_1d(skip2, x2.size(2))             # (B*T, 32, 120)
        x2 = x2 + skip2

        # ---- Block 3: 120 -> 240 ----
        x3 = F.gelu(self.norm_3(self.upsample_3(x2)))           # (B*T, 16, 240)
        x3 = F.gelu(self.temporal_refine_3(x3))                 # (B*T, 16, 240)

        skip3 = self.skip_to_x3(z_flat).unsqueeze(-1)           # (B*T, 16, 1)
        skip3 = self._bicubic_1d(skip3, x3.size(2))             # (B*T, 16, 240)
        x3 = x3 + skip3

        # ---- Block 4: 240 -> 480 ----
        x4 = F.gelu(self.norm_4(self.upsample_4(x3)))           # (B*T, 8, 480)
        x4 = F.gelu(self.temporal_refine_4(x4))                 # (B*T, 8, 480)

        # ---- Refinement and final conv ----
        refined = self.refine_stack(x4)                         # (B*T, 4, 480)
        features = self.final_conv(refined)                     # (B*T, 1, 480)

        mu = self.signal_mu(features).squeeze(1)                # (B*T, 480)
        logvar = self.signal_logvar(features).squeeze(1)        # (B*T, 480)
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.linear_2 = ResidualMLP(
            input_dim=256,
            hidden_dims=geometric_schedule(256, 512, 5),
            final_activation=True,
            use_skip_connection=True, 
            activation=nn.GELU,
        )
        
        self.linear_3 = ResidualMLP(
            input_dim=512,
            hidden_dims=geometric_schedule(512, 1200, 5),
            final_activation=True,
            use_skip_connection=True, 
            activation=nn.GELU,
        )
        
        self.skip_z_linear_2  = nn.Linear(latent_dim, 512)
        self.skip_z_linear_3 = nn.Linear(latent_dim, 1200)
        self.skip_linear_2_3 = nn.Linear(512, 1200)
        self.st_inv_module = InvStModule(latent_dim=latent_dim)
    ...
    def forward(
        ...
        """
        z_1 = self.linear_1(latent_z)
        z_2 = self.linear_2(z_1) + self.skip_z_linear_2(latent_z)
        st_ph_coefs = self.linear_3(z_2) + self.skip_z_linear_3(latent_z) + self.skip_linear_2_3(z_2)

        mu, logvar = self.st_inv_module(st_ph_coefs, latent_z)
        return st_ph_coefs, mu, logvar
...
class SeqVae(nn.Module):
    ...
    def forward(
        ...
        """
        mu_x = self.source_encoder(x_ph)  # (B, T, latent_dim)
        mu_y, logvar_y_prior, c_logvar = self.target_encoder(y_st, y_ph)  # (B, T, latent_dim) all of them
    ...
    def average_raw_prediction(self, mu_pr: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            ...
            (e.g., before warmup or beyond available windows) are filled with NaN.
        """
        if mu_pr.dim() != 3:
            raise ValueError(f"mu_pr must be 3D (B, T, H); got {mu_pr.shape}")

        B, T, H = mu_pr.shape
        s = int(self.decimation_factor)
        ...
        valid_ts = torch.arange(warmup, T, device=device)
        if valid_ts.numel() == 0:
            return avg

        # Build index grid for gathering raw targets
        start_idx = valid_ts[:, None] * s                     # (T_valid, 1)
        h_idx = torch.arange(H, device=device)[None, :]       # (1, H)
        idx = start_idx + h_idx                               # (T_valid, H)
        mask = idx < raw_len                                  # (T_valid, H) bool
        idx_clamped = idx.clamp(max=raw_len - 1)

        pred = mu_pr[:, valid_ts, :]                          # (B, T_valid, H)
        masked_pred = pred * mask.unsqueeze(0)
        ...
        flat_pred = masked_pred.reshape(B, -1)
        flat_mask = mask.unsqueeze(0).expand(B, -1, -1).reshape(B, -1).to(dtype)

        pred_sum.scatter_add_(1, flat_idx, flat_pred)
    ...
    def compute_loss(
        ...
        *,
        ...
        Args:
            ...
            y_ph (torch.Tensor): Target phase harmonic features, shape $(B, T, 44)$.
            y_raw (torch.Tensor): Raw target signal for reconstruction, shape $(B, R)$
                or $(B, R, 1)$ where $R$ is the raw length (e.g., 4800). If 3D with last
                dim=1, it will be squeezed to $(B, R)$.
        ...
        Returns:
            Dict[str, torch.Tensor]: Dictionary containing all loss components (all scalars):
                ...
                - total_loss: nll_loss + $\beta$ * kld_loss.
        """
        ...
        mu_pr = forward_outputs.get("mu_pr")   # (B, T, H)
        logvar_pr = forward_outputs.get("logvar_pr")

        # Apply warmup on decimated time steps before scoring reconstruction
        warmup = int(self.warmup_period)
        s = int(self.decimation_factor)
        ...
        if mu_pr is not None and logvar_pr is not None:
            ...
            # Maximum number of valid steps that have a full horizon available
            ...
            if start_t < max_valid_t:
                ...
                # Build target windows from raw: for each step t, take raw[t*s : t*s+H]
                start_idx = valid_steps * s                     # (T_valid,)
                h_idx = torch.arange(H, device=device)          # (H,)
                idx = start_idx[:, None] + h_idx[None, :]       # (T_valid, H)

                # Mask out incomplete windows near the end
                ...
                pred = mu_pr[:, valid_steps, :]
                lv = logvar_pr[:, valid_steps, :]
                var = lv.exp()

...
Encoder dropout does not break any of it, and that is worth stating because it is the non-obvious
part of this architecture rather than a restatement of the sibling's. Both the prior and the
posterior are read off **one common encoder forward** -- ``h_y`` is computed once and handed to
``prior_head`` and ``posterior_head`` alike -- so whatever activations dropout removes, it removes
from both branches identically. Two encoder passes would break the identity even with the deltas at
zero.
...
wrong. The perturbation test at the bottom is what makes the others meaningful, and every other
test in this package that claims to check KL behaviour perturbs first.
"""
...
def _closed_form_kl(out: dict) -> torch.Tensor:
    ...
    Written out here rather than called from the model, so the assertion is against the formula
    and not against the model agreeing with itself.
    """
    return 0.5 * (
        out["logvar_prior"]
        - out["logvar_post"]

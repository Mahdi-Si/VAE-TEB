...
**Nothing here is target-domain specific**, and the divergence manifest classifies the module
...
from teb_vae.lag_attn_cfs.eval.analyses import time_to_delivery as analysis
...
def test_both_reduced_columns_are_on_this_cells_per_sample_table(task, perturb_posterior) -> None:
    ...
    with a confident method sentence beside it -- which is the failure mode no amount of testing
    the arithmetic on hand-built frames can see."""
    import torch
    ...
    assert set(analysis.VALUE_COLUMNS) <= set(readout.columns)
...
# =============================================================================
@pytest.mark.slow
def test_the_real_run_bins_the_generated_epochs(collected_run) -> None:
    ...
    block = collected_run["summary"]["results"]["time_to_delivery"]
    trajectory = pd.read_csv(
        collected_run["results_dir"] / analysis.ANALYSIS_DIRNAME / analysis.TRAJECTORY_FILENAME
    )

...
class TrajectoryAnalyzer:
    ...
    def __init__(
        ...
        Args:
            ...
            dataloader: Provides batches with fhr_st, fhr_ph, up_st, up_ph, guid, epoch
    ...
    def _collect_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        with torch.no_grad():
            for batch in tqdm(self.dataloader, desc="Collecting data"):
                # Lag-attn v1 inputs: (fhr_st, fhr_ph, u_stream) where
                # u_stream = concat(up_st, up_ph) if use_up_st else up_ph.
                y_st = getattr(batch, "fhr_st", None)
                y_ph = getattr(batch, "fhr_ph", None)
                up_ph = getattr(batch, "up_ph", None)
                up_st = getattr(batch, "up_st", None)

                if y_st is None or y_ph is None or up_ph is None:
                    continue

                B = y_st.shape[0]

                model_use_up_st = bool(getattr(self.model, "use_up_st", True))
                if model_use_up_st:
                    if up_st is None:
                        continue
                    u_stream = torch.cat(
                        [up_st.to(self.device), up_ph.to(self.device)], dim=-1
                    )
                else:
                    u_stream = up_ph.to(self.device)
...
def run_trajectory_analysis(
    ...
    Args:
        model: VAE-TEB model
        dataloader: DataLoader with fhr_st, fhr_ph, up_st, up_ph, guid, epoch
        output_dir: Where to save outputs
        time_range_hours: Analyze this many hours before birth (None = all)

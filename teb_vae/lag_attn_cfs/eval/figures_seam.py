...
**The lag axis is this package's own, and it is not the sibling's.** A lag index $\ell$ is not
seconds, and here it is not physiological time either: the axis is *stored-coefficient* time,
uncorrected for the one-sided bank's composed group delay. So the label bound here is
:data:`~teb_vae.lag_attn_cfs.eval.lag_axis.COEFFICIENT_LAG_AXIS_LABEL` rather than the raw cells'
``COMPENSATED_LAG_AXIS_LABEL``, and it is drawn from :mod:`~teb_vae.lag_attn_cfs.eval.lag_axis` so
that a figure drawn through this seam, the caveat travelling beside it and the number reported
under it all come from one place. Importing the sibling's label here is the single edit that would
put the wrong claim on every lag figure in this package while changing no arithmetic at all.
"""
...
#: The cost of owning it here is stated rather than hidden: an evaluation figure of a cohort is
#: **not** the same colour as a training-callback figure of that cohort, and a reader putting the
#: two side by side must read the legend rather than the hue.
#:

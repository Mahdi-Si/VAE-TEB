...
* **Only the two saturation fractions need a masked recomputation.** In this model the
  ...
  pipeline's framing. Both are emitted and they may legitimately disagree.
...
def test_a_pinned_prior_variance_is_detected_while_the_decoder_looks_healthy(
    ...
    # asserted at exactly zero: a handful of raw samples land inside the margin at random init,
...
def test_the_verdict_carries_the_margin_that_defines_pinned(evaluated) -> None:
    ...
    verdicts = {
        ...
        for verdict in evaluated["summary"]["results"]["verdicts"]
    }
    values = verdicts["prior_variance_not_pinned"]["values"]
    bounds = evaluated["summary"]["collection"]["bounds"]
...
def test_on_a_real_run_the_spectrum_sums_to_the_kl_it_decomposes(evaluated) -> None:
...
def test_the_real_run_reports_every_diagnostic_with_its_meaning(evaluated) -> None:
    rows = evaluated["summary"]["results"]["latent"]["diagnostics"]

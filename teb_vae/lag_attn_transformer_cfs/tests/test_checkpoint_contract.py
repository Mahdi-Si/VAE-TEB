...
this model from the five it sits beside -- every one of them shares tensor names with it -- and buys
...
def _wrapped(cls, kwargs, task_cls=SeqVaeLagAttnTrfCfsTask):
...
def blob():
    ...
    return _lightning_style_checkpoint(_wrapped(SeqVaeLagAttnTrfCfs, _kwargs()))
...
def test_the_keep_index_is_the_only_record_of_the_decoder_width(blob):
    """The field this model cannot be rebuilt without. ``decoder_out_channels`` is not a keyword of
    this constructor at all: the width is derived from the gate, so a second field recording it
    could disagree with the gate and one of the two would be believed."""
...
def test_a_checkpoint_round_trips_into_a_fresh_model(tmp_path):
    ...
    check_model_class(saved, "SeqVaeLagAttnTrfCfs")
    rebuilt = SeqVaeLagAttnTrfCfs(**saved["model_kwargs"])
...
def test_a_checkpoint_reloads_with_no_shard_present(tmp_path, monkeypatch):
    """The property the whole ``model_kwargs`` stamp exists for, and the one this target domain
...
def test_the_class_guard_accepts_this_model_and_rejects_the_siblings(blob):
    check_model_class(blob, "SeqVaeLagAttnTrfCfs")  # must not raise

    for foreign in ("SeqVaeLagAttnRws", "SeqVaeLagAttnFs", "SeqVaeLagAttnCfs",
                    "SeqVaeLagAttnTrfRws", "SeqVaeLagAttnTrfFs"):
...
def test_a_foreign_blob_with_the_guard_skipped_refuses_rather_than_partly_succeeding():
    ...
    transformer = SeqVaeLagAttnTrfCfs(**_kwargs())
...
def test_a_checkpoint_from_another_warm_up_budget_is_refused():
    ...
    other_blob = _lightning_style_checkpoint(_wrapped(SeqVaeLagAttnTrfCfs, other_kwargs))

    check_model_class(other_blob, "SeqVaeLagAttnTrfCfs")  # same class: the guard cannot help
    assert other_blob["model_kwargs"]["target_keep_index"] == _OTHER_KEEP_INDEX

    shipped_width_model = SeqVaeLagAttnTrfCfs(**_kwargs())
    ...
    rebuilt = SeqVaeLagAttnTrfCfs(**other_blob["model_kwargs"])

...
### 7.4 The classification readout, and the analysis window it is measured over

Added 2026-09-08, on the operator's request, after the first production run. The three figures of §7.2 answer whether the latent moved and where the cohort sits in it; none of them answers *how well the score separates the two outcome groups*, which is the question the pilot was run to ask. Five figures and two tables now do, and they are drawn from what the evaluation stage already writes.

| Figure | What it shows | What it is not |
|---|---|---|
| 4 — `figure4_roc_pr` | Held-out ROC and precision-recall for the final-hour bags, both models overlaid, with AUROC/AP and their bootstrap intervals and each model's validation-selected operating point marked | The curves are threshold-free; the dot is the only place a threshold enters. PR chance is the adverse-outcome prevalence, not one half |
| 5 — `figure5_confusion` | Confusion counts at each model's threshold on one shared colour scale, with sensitivity, specificity, precision, NPV, $F_1$, accuracy and balanced accuracy beside them | Counts are recordings. `precision`, `npv` and `accuracy` carry no interval, because the paired bootstrap resamples `evaluate.METRIC_NAMES` and those three are outside it |
| 6 — `figure6_metrics_vs_time` | AUROC, average precision, $F_1$ and balanced accuracy per trajectory bin, with bands, the shaded supervised window, and adverse/total counts under every bin | Applying a frozen final-hour head outside its window. A metric moving towards delivery may be a trend or a change in who was still recorded |
| 7 — `figure7_roc_by_time_bin` | One ROC panel per bin that carried both groups, nearest delivery first | A bin with no adverse recording has **no panel**, so the panel count is itself a coverage statement |
| 8 — `figure8_confusion_counts_vs_time` | The four confusion cells per bin, stacked, one row per model | What the rates of figure 6 are rates *of*: a sensitivity on two adverse recordings and one on thirty read alike as a rate |

Three properties hold across all five, and the report states each rather than leaving it to a caption. The classifier is the **frozen final-hour head**, never refitted per bin. The threshold is the one **selected on validation**, never re-chosen on the held-out split or inside a bin. And the per-bin cohort is **whoever was observed in that bin** — coverage thins backwards, so the counts printed beside every number are part of the reading, and a bin carrying one outcome group is drawn as a gap rather than as a zero.

None of this requires refitting. `stage_report` reads `per_recording_test.parquet` (outcomes and one `logit_<model>` column each), `per_recording_bin.parquet` (the frozen head's score per recording per bin) and the thresholds in `results.json`, and recomputes every number from them — so a run finished before these figures existed gains them by regenerating its report, with no shard reopened, no checkpoint loaded and nothing selected a second time. It writes `classification_metrics.csv`, `per_bin_metrics.parquet` and `roc_curves_by_bin.parquet` beside the figures.

**`windows.analysis_hours`** separates what the objective optimises from what the run measures. Until now `windows.preservation_hours` was one knob driving the loader's coarse epoch bound, the extraction's keep-mask, the trajectory bins, the teacher term and every figure axis, so analysing six hours meant preserving six hours — a different experiment, not a longer view of the same one. `analysis_hours` is `null` by default, meaning *the same as `preservation_hours`*, which is exactly what every earlier run did. Set larger, the extraction, the bins, the per-bin discrimination and the figures reach that far back while the classification loss still supervises the final `supervised_hours` and `train.build_plans` narrows the teacher term back to `preservation_hours`. It may not be set smaller: the teacher term is computed on extracted anchors, so a narrower analysis window would leave the loss without its support. It is not free — the extraction also runs once per epoch on validation, so doubling it roughly doubles the per-epoch cost of a fit.
...
| LP-17 | Write the classification readout and the analysis window | LP-13, LP-15 | DONE | Requested by the operator after the first production run; §7.4 states what the figures do and do not establish. `latent_pilot/evaluate.py` (`CONFUSION_CELLS`, `RATE_NAMES`, `confusion_counts`, `derived_rates`, `roc_points`, `pr_points`, `SINGLE_CLASS_NOTE`, `metric_intervals`, `_draw_rows`, `_percentile_interval`, extended `recording_metrics` and `METRIC_NAMES`), `latent_pilot/analyze.py` (`bin_classification`, `bin_roc_points`, `BIN_METRIC_KEYS`), `latent_pilot/report.py` (five figures, their captions, `_render_with_caption`, `_classification_section`, `_time_section`), `latent_pilot/run.py` (the three derived-table constants and the `stage_report` block), `latent_pilot/config.py` (`windows.analysis_hours`, `analysis_hours`, the three cross-checks, the `settings_differences` tolerance), `latent_pilot/train.py` (`build_plans` narrows the teacher term to `preservation_hours`), `latent_pilot/configs/pilot.yaml`. **Two defects found and fixed along the way**, both in code this task touched: `stage_state.json` recorded `selection_locked: false` on a locked run, because the runner persisted the record it had held since `open_run` over the one `lock_selection` wrote under it (`config._carry_forward_flags`, now called by `mark_completed` and `mark_failed`) — the smoke scenario, run here for the first time, is what caught it; and the report layer's axis argument was named `preservation_hours` while spanning the analysis window, now `window_hours` in `signed_bin_hours` and `figure_trajectories`, with `_temporal_section`'s heading following the analysis window too. Tests: `latent_pilot/tests/logic/test_classification.py` (22 new checks) plus additions to `test_settings.py`, `test_run_state.py`, `test_report.py`, `test_adaptation.py` and `test_adaptation_fit.py`, and eight renders added to `latent_pilot/tests/test_figures.py`. **Run here, on the repository's own `.venv`:** the whole `latent_pilot/tests` tree — **490 checks, 0 failed in 63 s**, including the end-to-end smoke scenario over the generated fixtures, which produced all nine figures and the three new tables; and one throwaway drive of `--stage report` over a run directory written in the *previous* revision's shape — no `analysis_hours` in its protocol, no confusion cells in its `results.json` — which rendered everything without refitting. The production stages, on clinical data, remain unrun here |

**Two deviations from the original protocol, both stated here because they change what a later reader may assume.** The classification metrics are computed by the **report** stage rather than the evaluation stage, so that a finished run gains them without a refit; they are derived presentation, and the evaluation stage's own artifacts are untouched. And `settings_differences` now tolerates a schema key added at its default, without which adding `analysis_hours` would have made every existing run directory un-re-reportable — the one operation the request turned on.
...
| `report` | `figures/figure1_latent_space`, `figures/figure1b_latent_space_by_coverage`, `figures/figure2_supervised_axis`, `figures/figure3_trajectories`, `figures/figure4_roc_pr`, `figures/figure5_confusion`, `figures/figure6_metrics_vs_time`, `figures/figure7_roc_by_time_bin`, `figures/figure8_confusion_counts_vs_time` (in `figure_format`, `pdf` by default); `classification_metrics.csv`, `per_bin_metrics.parquet`, `roc_curves_by_bin.parquet`; and `report.md`. The three tables and the last five figures are **derived**: every number in them is recomputed from the held-out scores and the locked thresholds this stage reads, which is what lets a finished run gain them without refitting |
...
* **Re-reporting** names the directory *without* `resume`. Only `tests`, `smoke`, `preflight` and `report` may run that way; they rewrite presentation and touch no fitted artifact. `report` re-renders the figures and `report.md` from `results.json` and needs neither the shards nor the checkpoint, so it works on a machine where the data is no longer mounted. It also **recomputes the §7.4 classification readout** from the held-out scores already in the directory, so a run finished before those figures existed gains them here.
* **A run finished before a settings key existed** re-reports unchanged. `settings_differences` treats a dotted path that is absent from the stored protocol and carries the schema's own default as a *schema addition* rather than a changed choice, and names it in the log. A path present in both with different values, and a path added and then set to something other than its default, both still refuse.

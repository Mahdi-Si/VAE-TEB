...
from model.vae_teb_prediction.model.model_raw.testing.metrics import project_kld_per_dim
from model.vae_teb_prediction.model.model_raw.testing.plot_single_samples import (
    ...
    *,
    ...
    with runner.inference_mode():
        for batch in runner.iter_batches(loader, max_samples):
            ...
            # Raw port: batches carry no scattering features -- guard them all (``None`` on raw)
            # so the plotter draws the raw fhr/up trace + KLD panels and skips the feature panel.
            fhr_st_np = None
            if hasattr(batch, "fhr_st") and isinstance(batch.fhr_st, torch.Tensor):
            ...
            batch_size = int(attn_np.shape[0])  # raw-safe: no fhr_st on raw batches
            for idx in range(batch_size):
                ...
                try:
                    plot_sample_signals_kld(
                        ...
                        fhr_st=fhr_st_np[idx] if fhr_st_np is not None else None,

...
1. A causal conv-Transformer encoder builds the target history state $H^y_t$ over its full causal
   prefix. The source stream is projected and then read through whichever memory
   ``lag_kv_source`` selects -- its own windowed encoder by default, or the convolution stem
   alone -- and either way that memory is deliberately *local*, so a source state summarises a
   neighbourhood and the lag attention below keeps its ability to tell adjacent delays apart.
...
* **No decoder bypass.** The decoder's forward accepts $z$ and, on a cell that builds the
  ...
  additional source-derived predictive information. The persistence input does not weaken that. It
  is target-only, both invocations receive the identical tensor, and it enters the mean head alone
  -- so it cancels exactly out of the base-minus-full gap the coupling is read from.
...
from teb_vae.lag_attn.nets.encoders import LAG_KV_SOURCE_CHOICES
...
class SeqVaeLagAttnTrfRws(nn.Module):
    ...
    #: delegation sites read it through :func:`getattr` with a ``None`` default, and ``None`` there
    #: means the score skips the multiplication rather than multiplying by ones.
        ...
        *,
        ...
        Args:
            ...
            lag_kv_source: Which source representation the lag attention's keys **and** values are
                built from. ``'encoder'`` (the default) is the deep history state $H^U$ -- the model
                this family has always been -- and is bitwise what was built before the keyword
                existed. ``'conv_stem'`` builds a
                :class:`~teb_vae.lag_attn_transformer_rws.nets.encoders.GatedCausalConvStem` over
                the same stem schedule instead, so a key at lag $\ell$ summarises a bounded
                neighbourhood of $\ell$; ``'adapter'`` uses the projected stream itself, one step of
                reach. Under either local arm the deep source encoder is **not built at all** --
                nothing else in the model consumes it -- so ``source_attention_blocks`` and
                ``source_attention_window`` describe the ``'encoder'`` arm only, and those arms are
                smaller than the default by the whole source attention stack. Why a deep state is
                the wrong memory to attend over, and why keys and values must move together, is in
        ...
        # encoder -- the whole windowed attention stack -- is not constructed at all: nothing else
        # in the model reads it, so building one would be a starved parameter block in DDP's
        # expectation set and a claim in the manifest that the model attends over a state it does
        # not have.
        ...
        if self.lag_kv_source == "encoder":
            ...
            # The same stem schedule as the encoder's, so the two arms differ in what is *removed*
            # rather than in two independently chosen front ends.
            self.source_kv_stem = GatedCausalConvStem(
                d_model=d_model,
                conv_kernels=encoder_conv_kernels,
                conv_dilations=encoder_conv_dilations,
                dropout=self.source_dropout,
    ...
    def source_kv_body(self) -> Optional[nn.Module]:
        ...
        -- the forward, the two source controls and the causal cells' prior clock -- goes through
        here or through :meth:`encode_source_kv`, so an arm cannot come to mean one thing in the
        forward and another in a readout of it.

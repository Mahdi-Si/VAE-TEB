...
Under ``gaussian_nll`` the decoder emits a mean **and** a log-variance per raw sample, and the
block score is a negative log density only if the second one is true. Nothing else in this
...
Four readings, over the raw samples themselves:
...
* **CRPS**, in closed form for a Gaussian. Proper, bounded, and in the units of the signal -- so
  unlike the NLL it can be quoted in bpm.
...
#: :data:`~teb_vae.lag_attn_rws.eval.metrics.DEFAULT_PINNED_VARIANCE_MAX_FRAC`, and the gap
...
def logvar_frame(report: Dict[str, Any]) -> pd.DataFrame:
    ...
    Returns:
        One row per bin, with the fraction of scored raw samples in it.
    """
...
def run_calibration_analysis(
    ...
    *,
    ...
    crps, unit = sigma_to_bpm(
        [float(report.get("crps_normalised", float("nan")))],
        dict(record.get("normalization") or {}),
    )
    written = [
    ...
    return {
        ...
        "n_raw_samples": int(report.get("n_raw_samples", 0)),
        ...
        "crps": float(crps[0]),
        "crps_unit": unit if unit != NORMALISED_UNIT else NORMALISED_UNIT,

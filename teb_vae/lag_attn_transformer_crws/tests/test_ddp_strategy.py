...
# The claim
# --------------------------------------------------------------------------------------
def test_the_shipped_config_earns_every_parameter_reachable(trainer) -> None:
...
def test_a_single_device_needs_no_strategy(trainer) -> None:
    assert trainer.select_ddp_strategy(1, trainer.config) == "auto"


def test_the_smoke_configs_mse_selects_the_fallback(trainer) -> None:
...
def test_the_buffer_broadcast_is_off_and_the_gradients_are_bucket_views(trainer) -> None:
...
def test_no_buffer_is_a_running_statistic_so_the_broadcast_is_safe_to_skip() -> None:
    ...
    This input domain adds **two** buffers of its own -- the two per-block source warmth patterns --
    against the causal-feature cells' three: ``warm_tertile_id`` partitions kept *target* channels
    and this target has none, so it must be absent rather than empty. Both are functions of the
    resolved budget and the geometry, so they belong in the same category. They are non-persistent
    for a second reason: their contents follow the budget, so a persistent copy would make a
    checkpoint trained at one budget fail to load at another and report it as misaligned keys rather
    than as a budget mismatch."""
    torch.manual_seed(0)
...
def test_static_graph_is_not_claimed(trainer) -> None:
...
def test_the_settings_reach_the_strategy_object(trainer) -> None:
...
def test_the_selector_is_a_pure_function_of_config(trainer) -> None:
...
def test_the_hook_is_the_un_prefixed_name_the_framework_looks_up() -> None:
    ...
    assert "_select_ddp_strategy" not in vars(LagAttnTrfCrwsTrainer)

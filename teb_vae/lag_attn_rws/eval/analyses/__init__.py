...
vector sidecar, the retained heavy arrays, and the readouts themselves -- is what an analysis
reads, through :class:`AnalysisContext`. That is what makes ``--only <anything>`` against a
finished run directory work with no checkpoint loaded and no GPU, which is the whole point of
splitting collection from emission. It is also why the layering test forbids ``analyses/*`` from
importing Lightning, ``model.*``, or this package's ``task`` / ``trainer`` / ``plotting``.
...
* ``n_samples`` -- how many segments this analysis actually scored, or ``None`` when it scored
  none because it describes something other than a population (the input channel map is the
...
class AnalysisContext:
    ...
    per-sample diagnostic pages. A page is the *whole* forward output of one segment -- eleven
    tensors, three megabytes -- and the extreme-value pages are chosen by sorting a table the pass
    had not written when it ran, so neither can be served by retention. Both are ``None`` on a
    pass with no checkpoint, and the analysis that wants them records a skip rather than assuming
    them; every other analysis reads the tables, and the layering test still forbids
    ``analyses/*`` from *importing* Lightning, ``model.*`` or this package's ``task`` / ``trainer``
    / ``plotting``.

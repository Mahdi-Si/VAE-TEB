...
the held-out k-fold causal shards rather than the healthy-only pretraining split, the statistics
file accumulated from those same shards, four extra ``load_fields`` the clinical questions are
asked in, and an ``eval_config`` block. Those arrive as a small committed delta deep-merged
**over** the run's own resolved config, so the run's
...
#:
#: Three names are deliberately **absent**. ``alpha``: an operator who could widen the significance
...
#: trend against time before delivery is visible. ``anchor_stride``: the evaluation decodes the
#: dense anchor set and nothing else, and an operator who could set the stride could change the
#: population every number in the run is computed over.
...
#: of the three equal to its counterpart once ``metrics.py`` exists, so a change there fails a test
#: instead of drifting.
#:
#: ``clock_margin_min_nats`` ships **unset**, and that is the setting rather than an omission. It
#: is the margin the availability-clock difference
#: $\Delta_{\mathrm{clock}} = \texttt{source\_conditioned\_kl\_raw} - \texttt{kld\_source\_null}$
#: must clear for ``coupling_exceeds_availability_clock`` to PASS, and where that boundary sits is
#: what the first production runs are supposed to *measure*. A provisional number would decide a
#: FAIL on exactly the run that was going to answer the question, and nobody could tell from the
#: output whether a healthy model had failed or a broken one had passed. At ``None`` the verdict is
#: INCONCLUSIVE and the measured difference, both interval ends and the positive fraction are
#: emitted anyway, so the threshold can be set from data rather than from judgement.
...
#: Smallest margin the availability-clock verdict may be gated on, in nats per anchor. A threshold
#: of exactly zero passes on any non-negative difference and a negative one can never fail, so
#: either would report an active verdict that is not one; and below a millinat the threshold sits
#: inside the width of the bootstrap interval it is compared against, which is the same failure
...
def validate_eval_config(config: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    # Nullable, and validated only when set -- the ``max_samples`` shape, behind the same
    # ``is not None`` guard. ``None`` is not a missing value here but the shipped setting: it makes
    # ``coupling_exceeds_availability_clock`` INCONCLUSIVE while the measured difference and its
    # interval are emitted regardless, which is what lets the threshold be chosen from the first
    # production runs rather than guessed before them.
    if resolved["clock_margin_min_nats"] is not None:
        resolved["clock_margin_min_nats"] = _require_positive_float(
            resolved["clock_margin_min_nats"],
            "clock_margin_min_nats",

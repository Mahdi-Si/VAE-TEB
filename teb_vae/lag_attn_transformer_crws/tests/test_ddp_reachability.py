...
def test_the_attention_projection_is_frozen_out_of_the_expectation_set(task) -> None:
...
def test_the_availability_terms_are_unconditional_in_the_forward() -> None:
...
# --------------------------------------------------------------------------------------
def test_the_phase_derivation_runs_no_collective() -> None:
    ...
    source = inspect.getsource(SeqVaeLagAttnTrfCrwsTask.anchor_phase)
    source += inspect.getsource(SeqVaeLagAttnTrfCrwsTask.resolve_anchor_geometry)
    source += inspect.getsource(SeqVaeLagAttnTrfCrwsTask._build_forward_inputs)
...
def test_the_anchor_tensor_shape_is_a_geometry_constant_at_every_phase(phase) -> None:
    ...
    # anchor -- which is what keeps the KL and reconstruction denominators from diverging, and here
    # additionally what keeps the anchored raw gather from pulling one window twice.
    for row in range(3):
